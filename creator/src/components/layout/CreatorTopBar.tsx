...
export const CreatorTopBar: React.FC<CreatorTopBarProps> = ({
  ...
  const persistMarkAllNotificationsRead = async () => {
    ...
    try {
      ...
      markAllAsRead();
      await queryClient.invalidateQueries({ queryKey: ['creator', 'notifications'] });
    } catch (error: any) {
      setNotificationActionError(error?.message || 'Failed to mark notifications as read');
    }
  };
  ...
  const persistNotificationRead = async (notificationId: string) => {
    ...
    try {
      ...
      setNotificationActionError(error?.message || 'Failed to mark notification as read');
    }
  };
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: { xs: 1, sm: 1.5 } }}>
          ...
          >
            <Box sx={{ p: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid', borderColor: 'divider' }}>
              ...
              {unreadCount > 0 && (
                <Button size="small" onClick={persistMarkAllNotificationsRead} sx={{ fontSize: '0.75rem' }}>
                  Mark all read
                </Button>
              )}
            </Box>
