...
export const GiftsPage: React.FC = () => {
  const giftsQuery = useQuery({
    ...
    queryFn: ({ signal }) => creatorApi.getRecentReceivedGifts(signal),
    staleTime: 30 * 1000,
    retry: 1,
  });
  ...
  if (giftsQuery.isError) {
    return (
      <PageErrorState
        ...
        message={giftsQuery.error?.message || 'Unable to retrieve virtual gift transaction logs.'}
      ...
      {gifts.length === 0 ? (
        ...
        <Grid container spacing={2.5}>
          {gifts.map((g: any, idx: number) => (
            <Grid key={g.id || idx} xs={12} sm={6} md={4}>
              <Card>
