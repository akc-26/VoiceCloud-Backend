  ...
  if (isError) {
    return (
      <PageErrorState
        ...
        message={plansQuery.error?.message || subscribersQuery.error?.message || 'Failed to fetch subscription data.'}
        onRetry={() => {
          plansQuery.refetch();
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            Define membership tiers, monthly coin prices, exclusive supporter badges, and member lists.
          </Typography>
        </Box>
        <Button
          variant="contained"
              ...
              <Card>
                <CardContent sx={{ p: 3 }}>
                  ...
                  <Stack spacing={1} sx={{ mb: 2.5 }}>
                    {(p.perks || ['VIP Badge', 'Priority Mic Seat', 'Custom Chat Bubble']).map((perk: string, i: number) => (
                  ...
                  {subscribers.map((sub: any) => (
                    <TableRow key={sub.id}>
                      <TableCell sx={{ fontWeight: 700 }}>
                        {sub.subscriber?.displayName || sub.subscriber?.username || 'Supporter'}
                      </TableCell>
                      <TableCell>{sub.plan?.name || 'Silver Supporter'}</TableCell>
                      <TableCell>{new Date(sub.startedAt || Date.now()).toLocaleDateString()}</TableCell>
                      <TableCell>
                        <Chip label={sub.status || 'ACTIVE'} color="success" size="small" />
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
      ...
      <Dialog open={isCreateOpen} onClose={() => setIsCreateOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ fontWeight: 800 }}>Create New Subscription Tier</DialogTitle>
        <DialogContent dividers>
          <Stack spacing={2.5} sx={{ mt: 1 }}>
            <TextField label="Tier Name" placeholder="e.g. Platinum All-Access" fullWidth />
            <TextField label="Monthly Coin Price" placeholder="1000" type="number" fullWidth />
            <TextField label="Included Perks (comma separated)" placeholder="VIP Badge, Priority Mic, Custom Chat" fullWidth />
          </Stack>
        </DialogContent>
        <DialogActions sx={{ p: 2.5 }}>
          <Button onClick={() => setIsCreateOpen(false)}>Cancel</Button>
          <Button variant="contained" onClick={() => setIsCreateOpen(false)} sx={{ fontWeight: 700 }}>
            Create Tier
          </Button>
        </DialogActions>
      </Dialog>
