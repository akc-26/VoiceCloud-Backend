...
export const SchedulePage: React.FC = () => {
  ...
  const createMutation = useMutation({
    mutationFn: (data: Record<string, any>) => creatorApi.createScheduledRoom(data),
    onSuccess: () => {
  ...
  const handleSaveSchedule = () => {
    ...
    const scheduledStartTime = new Date(`${scheduledDate}T${scheduledTime}:00Z`).toISOString();
              ...
              <Card>
                <CardContent sx={{ p: 3 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
                    <Chip label={evt.category || evt.type || 'Live Stream'} color="primary" size="small" />
                  ...
                  <Stack direction="row" spacing={2} sx={{ mb: 2 }}>
                    ...
                    <Typography variant="body2" color="text.secondary" sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                      <Clock size={16} />{' '}
                      {evt.scheduledStartTime
                        ? new Date(evt.scheduledStartTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
                        : evt.time || '20:00 UTC'}
                    </Typography>
                  </Stack>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <Typography variant="caption" color="text.secondary" sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                      <Users size={14} /> {evt.rsvpCount ?? evt.attendees ?? 0} RSVP Reminders
                    </Typography>
                    <IconButton
                      color="error"
      ...
      <Dialog open={isScheduleOpen} onClose={() => setIsScheduleOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions sx={{ p: 2.5 }}>
          ...
          <Button
            ...
            disabled={!title.trim() || !scheduledDate || !scheduledTime || createMutation.isPending}
            sx={{ fontWeight: 700 }}
          >
            {createMutation.isPending ? <CircularProgress size={20} /> : 'Save Schedule'}
          </Button>
        </DialogActions>
      </Dialog>
    </Box>
