...
import { UserCheck, Search, MessageSquare, UserPlus, Users } from 'lucide-react';
...
export const FollowersPage: React.FC = () => {
  ...
  const paginatedFollowers = filteredFollowers.slice((page - 1) * itemsPerPage, page * itemsPerPage);
  ...
  if (followersQuery.isError) {
    return (
      <PageErrorState
        ...
        message={followersQuery.error?.message || 'Unable to retrieve follower directory from backend.'}
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            Total Followers: <strong>{(profile.followersCount ?? 14250).toLocaleString()}</strong> | Engage with listeners and supporters.
          </Typography>
        </Box>
      ...
      {filteredFollowers.length === 0 ? (
        <EmptyState
          ...
          description={search ? `No followers matching "${search}".` : 'You do not have any followers yet.'}
        ...
        <>
          <Grid container spacing={2.5}>
            {paginatedFollowers.map((item, idx) => (
              <Grid key={item.id || idx} xs={12} sm={6} md={3}>
                <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                  <CardContent sx={{ p: 2.5, textAlign: 'center' }}>
                    <Avatar
                      src={item.avatarUrl}
                      sx={{ width: 56, height: 56, mx: 'auto', mb: 1.5, bgcolor: 'primary.main', fontWeight: 700 }}
                    >
                    ...
                    <Chip
                      ...
                      color={item.badge?.includes('VIP') ? 'secondary' : 'primary'}
                      size="small"
                      variant="outlined"
                      sx={{ mb: 1.5 }}
                    />
                    ...
                    <Typography variant="caption" color="text.disabled" sx={{ display: 'block', mb: 2 }}>
                      Followed {new Date(item.followedAt || Date.now()).toLocaleDateString()}
                    </Typography>
                    ...
                    <Button
                      ...
                      size="small"
                      fullWidth
                      startIcon={item.isFollowingBack ? <UserCheck size={16} /> : <UserPlus size={16} />}
                      sx={{ fontWeight: 700 }}
                    >
