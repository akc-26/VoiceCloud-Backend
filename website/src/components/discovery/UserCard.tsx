...
import { isConsumerDiscoverableUser } from '@/features/discovery/consumer-users';
...
export function UserCard({ user, action, onAction, busy }: { user: VoiceCloudUser; action?: string; onAction?: () => void; busy?: boolean }) {
  ...
  // Defense in depth: even if a future list forgets to pre-filter its data, a
  // consumer people card must never render the signed-in user, guest accounts,
  // or platform-management identities.
  if (!isConsumerDiscoverableUser(user, currentUser)) return null;
