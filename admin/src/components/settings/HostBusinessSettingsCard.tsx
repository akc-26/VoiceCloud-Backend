...
export const HostBusinessSettingsCard: React.FC = () => {
  ...
  const [settings, setSettings] = useState<HostBusinessSettings>(EMPTY_SETTINGS);
  ...
  const updateLevel = (
    index: number,
    patch: Partial<HostLevelSettings>,
  ) => {
    setSettings((current) => ({
      ...current,
...
function validateSettings(settings: HostBusinessSettings): string {
  ...
  for (let index = 0; index < settings.levels.length; index += 1) {
    ...
    if (
      index > 0 &&
      level.minimumXp <= settings.levels[index - 1].minimumXp
    ) {
