...
export const FeatureFlagsPage: React.FC = () => {
  ...
  return (
    <Box>
      <Box sx={{ mb: 3 }}>
        <Typography variant="h4" fontWeight={800}>Feature Flag Control Console</Typography>
      ...
      <Grid container spacing={2.5}>
        {flags.map((flag) => (
          <Grid item xs={12} md={6} key={flag.key}>
            <Card elevation={0}>
              <CardContent>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <FlagIcon color="primary" />
                    <Typography variant="h6" fontWeight={700}>{flag.name}</Typography>
                  </Box>
                ...
                <FormControlLabel
                  ...
                  label={<Typography variant="subtitle2" fontWeight={700}>{flag.isEnabled ? 'Active (ENABLED)' : 'Disabled'}</Typography>}
                />
              </CardContent>
            </Card>
          </Grid>
