...
export const TasksAchievementsPage: React.FC = () => {
  ...
  const [editingAchievement, setEditingAchievement] = useState<Partial<AchievementDefinition> | null>(null);

  const [seasonDialogOpen, setSeasonDialogOpen] = useState(false);
  const [editingSeason, setEditingSeason] = useState<Partial<SeasonalEvent> | null>(null);
  ...
  const fetchData = async () => {
    ...
    try {
      if (activeTab === 0) {
        ...
        const data = await tasksAchievementsAdminService.getAuditLogs({ limit: 50 });
  ...
  const handleSaveTask = async () => {
    ...
    try {
      if (editingTask.id) {
        await tasksAchievementsAdminService.updateTask(editingTask.id, editingTask);
        setSnackbarMessage('Task updated successfully');
      } else {
        const resetPeriod = activeTab === 1 ? 'weekly' : activeTab === 2 ? 'monthly' : 'daily';
  ...
  const handleSaveAchievement = async () => {
    ...
    try {
      if (editingAchievement.id) {
        ...
        await tasksAchievementsAdminService.createAchievement(editingAchievement);
  ...
  const handleInspectUser = async () => {
    ...
    try {
      const data = await tasksAchievementsAdminService.getUserProgress(inspectUserId);
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            Phase 28 Gamification Engine — Configure tasks, permanent achievements, XP levels, rewards, streaks & seasonal events.
          </Typography>
        </Box>
      ...
      <Paper sx={{ mb: 3 }}>
        <Tabs
          ...
          textColor="primary"
        >
          <Tab label="Daily Tasks" icon={<TaskAltIcon />} iconPosition="start" />
          <Tab label="Weekly Tasks" icon={<TaskAltIcon />} iconPosition="start" />
          <Tab label="Monthly Missions" icon={<TaskAltIcon />} iconPosition="start" />
          <Tab label="Achievements" icon={<EmojiEventsIcon />} iconPosition="start" />
          <Tab label="XP Levels" icon={<AssessmentIcon />} iconPosition="start" />
          <Tab label="Reward Audit Logs" icon={<CardGiftcardIcon />} iconPosition="start" />
          <Tab label="Seasonal Events" icon={<EmojiEventsIcon />} iconPosition="start" />
          <Tab label="Analytics Engine" icon={<AssessmentIcon />} iconPosition="start" />
        </Tabs>
      </Paper>
        ...
        <Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
            <Typography variant="h6" sx={{ fontWeight: 700 }}>
              {activeTab === 0 ? 'Daily Tasks' : activeTab === 1 ? 'Weekly Tasks' : 'Monthly Missions'}
            </Typography>
            <Box sx={{ display: 'flex', gap: 1 }}>
              <Button
                variant="outlined"
              ...
              <TableBody>
                {tasks.map((task) => (
                  <TableRow key={task.id}>
                    ...
                    <TableCell>
                      <Chip label={task.eventKey} size="small" variant="outlined" />
                    </TableCell>
                    <TableCell>{task.targetCount}</TableCell>
                    <TableCell>
                      <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap' }}>
                        {task.rewardCoins > 0 && <Chip label={`${task.rewardCoins} Coins`} color="warning" size="small" />}
                        {task.rewardDiamonds > 0 && <Chip label={`${task.rewardDiamonds} Dia`} color="primary" size="small" />}
                        {task.rewardXp > 0 && <Chip label={`${task.rewardXp} XP`} color="success" size="small" />}
                      </Box>
                    </TableCell>
                    <TableCell>
                      <Chip
                ...
                {tasks.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={6} align="center">
                      No tasks found for this period. Click "Create Task" to add one.
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
              ...
              <TableBody>
                {achievements.map((ach) => (
                  <TableRow key={ach.id}>
                    ...
                    <TableCell>
                      <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap' }}>
                        {ach.coinReward > 0 && <Chip label={`${ach.coinReward} Coins`} color="warning" size="small" />}
                        {ach.diamondReward > 0 && <Chip label={`${ach.diamondReward} Dia`} color="primary" size="small" />}
                        {ach.xpBonus > 0 && <Chip label={`${ach.xpBonus} XP`} color="success" size="small" />}
                      </Box>
                    </TableCell>
                    <TableCell>
                      <Chip
                  ...
                  {userProgressData && (
                    <Box sx={{ mt: 2 }}>
                      <Alert severity="info" sx={{ mb: 2 }}>
                        Level {userProgressData.xp?.level} ({userProgressData.xp?.levelTitle}) — {userProgressData.xp?.currentXp}/{userProgressData.xp?.requiredXp} XP ({userProgressData.xp?.progressPercent}%)
                      </Alert>
                      <Typography variant="subtitle2">
                        Streaks: {userProgressData.streaks?.map((s: any) => `${s.streakType}: ${s.currentStreak}d`).join(' | ')}
                      </Typography>
                    </Box>
                  )}
                </CardContent>
              ...
              <Card>
                <CardContent>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                    <Typography variant="body2">• Lvl 1–5: Novice Voice</Typography>
                    <Typography variant="body2">• Lvl 6–10: Bronze Speaker</Typography>
                    <Typography variant="body2">• Lvl 11–15: Silver Host</Typography>
                    <Typography variant="body2">• Lvl 16–20: Gold Broadcaster</Typography>
                    <Typography variant="body2">• Lvl 21–25: Platinum Vocalist</Typography>
                    <Typography variant="body2">• Lvl 26–30: Diamond Superstar</Typography>
                    <Typography variant="body2">• Lvl 31–35: Master Maestro</Typography>
                    <Typography variant="body2">• Lvl 36–40: Grandmaster Idol</Typography>
                    <Typography variant="body2">• Lvl 41–45: Legend Icon</Typography>
                    <Typography variant="body2">• Lvl 46–50: Mythic Voice</Typography>
                  </Box>
                </CardContent>
              </Card>
            </Grid>
                ...
                {seasons.map((season) => (
                  <TableRow key={season.id}>
                    <TableCell>
                      ...
                      <Typography variant="caption">{season.description}</Typography>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell align="right">
                      <Button
                        size="small"
                        color="warning"
                        onClick={() => tasksAchievementsAdminService.triggerRollover(season.id)}
                      >
                        Rollover
                      </Button>
                    </TableCell>
      ...
      <Dialog open={taskDialogOpen} onClose={() => setTaskDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>{editingTask?.id ? 'Edit Task' : 'Create Task'}</DialogTitle>
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 1 }}>
            ...
            <TextField
              ...
              onChange={(e) => setEditingTask({ ...editingTask, eventKey: e.target.value })}
            ...
            <TextField
              ...
              onChange={(e) => setEditingTask({ ...editingTask, targetCount: parseInt(e.target.value) || 1 })}
            />
            <Grid container spacing={2}>
              <Grid item xs={4}>
                <TextField
                  ...
                  onChange={(e) => setEditingTask({ ...editingTask, rewardCoins: parseInt(e.target.value) || 0 })}
                />
              </Grid>
              <Grid item xs={4}>
                <TextField
                  ...
                  onChange={(e) => setEditingTask({ ...editingTask, rewardDiamonds: parseInt(e.target.value) || 0 })}
                />
              </Grid>
              <Grid item xs={4}>
                <TextField
                  ...
                  onChange={(e) => setEditingTask({ ...editingTask, rewardXp: parseInt(e.target.value) || 0 })}
                />
              </Grid>
            </Grid>
          </Box>
      ...
      <Dialog open={achievementDialogOpen} onClose={() => setAchievementDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>{editingAchievement?.id ? 'Edit Achievement' : 'Create Achievement'}</DialogTitle>
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 1 }}>
            <TextField
              label="Title"
            ...
            <TextField
              ...
              onChange={(e) => setEditingAchievement({ ...editingAchievement, eventKey: e.target.value })}
            />
            <TextField
              label="Target Count"
              type="number"
              ...
              onChange={(e) => setEditingAchievement({ ...editingAchievement, targetCount: parseInt(e.target.value) || 1 })}
            />
          </Box>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={grantRewardOpen} onClose={() => setGrantRewardOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 1 }}>
            ...
            <TextField
              ...
              onChange={(e) => setGrantData({ ...grantData, rewardType: e.target.value })}
            >
            ...
            <TextField
              ...
              onChange={(e) => setGrantData({ ...grantData, amount: parseInt(e.target.value) || 1 })}
            />
            <TextField
              label="Reason"
              fullWidth
