...
export const AuthManagementPage: React.FC = () => {
  ...
  const fetchSettings = async () => {
    setLoading(true);
    try {
      const res = await api.get<SystemSetting[]>('/admin/settings/group/authentication');
      const settings = res.data || [];
      settings.forEach((s) => {
        if (s.key === 'jwt_expiration') setJwtExpiration(s.value);
        if (s.key === 'jwt_refresh_expiration') setJwtRefreshExpiration(s.value);
        if (s.key === 'allow_guest_login') setAllowGuestLogin(s.value === 'true');
        if (s.key === 'allow_google_login') setAllowGoogleLogin(s.value === 'true');
        if (s.key === 'allow_phone_login') setAllowPhoneLogin(s.value === 'true');
        if (s.key === 'require_referral_code') setRequireReferralCode(s.value === 'true');
        ...
        if (s.key === 'failed_login_lockout_attempts') setLockoutAttempts(s.value);
        if (s.key === 'failed_login_lockout_duration') setLockoutDuration(s.value);
      });
    } catch {
  ...
  const handleSaveAll = async () => {
    ...
    await handleSaveSetting('require_referral_code', String(requireReferralCode));
    ...
    addToast('success', 'All Authentication Platform settings saved successfully');
  };
    ...
    <Box>
      <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Box>
          <Typography variant="h4" sx={{ display: 'flex', alignItems: 'center', gap: 1, fontWeight: 800 }}>
            <SecurityIcon color="primary" fontSize="large" /> Authentication & Identity Console
          </Typography>
          <Typography variant="body2" color="text.secondary">
            Manage Phase 16 JWT tokens, OAuth, Phone OTP, Guest access policies, Device limits, and Lockout rules
          </Typography>
        </Box>
        <Button variant="contained" color="primary" onClick={handleSaveAll} size="large">
          Save Changes
        </Button>
      </Box>
      ...
      <Box sx={{ borderBottom: 1, borderColor: 'divider', mb: 3 }}>
        <Tabs value={activeTab} onChange={(_, val) => setActiveTab(val)}>
          <Tab label="Authentication Policies" icon={<KeyIcon />} iconPosition="start" />
          <Tab label="Login Methods" icon={<PhoneAndroidIcon />} iconPosition="start" />
          <Tab label="Security & Lockouts" icon={<LockClockIcon />} iconPosition="start" />
          <Tab label="Active Sessions & History" icon={<DevicesIcon />} iconPosition="start" />
        </Tabs>
      </Box>
                    ...
                    <FormControlLabel
                      control={
                        <Switch
                          ...
                          onChange={(e) => setRequireReferralCode(e.target.checked)}
                          color="primary"
                        />
                      }
                      label={
                        <Box>
                          ...
                          <Typography variant="caption" color="text.secondary">
                            When enabled, users must provide a valid referral code to sign up.
                          </Typography>
                        </Box>
                      }
                    />
            ...
            <Card elevation={0}>
              <CardContent>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                  ...
                  <Typography variant="h6" sx={{ fontWeight: 700 }}>Phone OTP Auth</Typography>
                </Box>
            ...
            <Card elevation={0}>
              <CardContent>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                  ...
                  <Typography variant="h6" sx={{ fontWeight: 700 }}>Google Sign-In</Typography>
                </Box>
                <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            ...
            <Card elevation={0}>
              <CardContent>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                  ...
                  <Typography variant="h6" sx={{ fontWeight: 700 }}>Guest Access</Typography>
                </Box>
                <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                    ...
                    <TableRow key={row.id}>
                      ...
                      <TableCell>
                        <Chip
                          label={row.action}
                          size="small"
                          color={row.action === 'LOGIN' ? 'success' : row.action === 'FAILED_LOGIN' ? 'error' : 'default'}
                        />
                      </TableCell>
