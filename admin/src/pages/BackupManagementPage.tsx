...
export const BackupManagementPage: React.FC = () => {
  ...
  const [newBackupDto, setNewBackupDto] = useState({
    ...
    isEncrypted: true,
    notes: `Platform snapshot created via ${BRAND_CONFIG.products.admin.shortName}`,
  });
  ...
  const [selectedBackupForRestore, setSelectedBackupForRestore] =
    useState<any>(null);
  const [restoreTargetComponents, setRestoreTargetComponents] = useState<
    string[]
  >(['database', 'redis', 'storage', 'config']);
  ...
  const handlePurgeRetention = async () => {
    try {
      ...
      addToast(
        'info',
        `Retention cleanup purged ${res.deletedCount} expired backups`,
      );
      void fetchAllData();
    } catch {
    ...
    <Box sx={{ p: 1 }}>
      ...
      >
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            Full platform snapshotting, DB table export, Redis state caching,
            AES-256 encrypted archives, and automated disaster recovery
          </Typography>
        </Box>
          ...
          <Grid size={{ xs: 12, sm: 6, md: 3 }}>
            ...
            >
              ...
              >
                ...
                <Chip
                  label={drStatus.status}
                  color={
                    drStatus.status === 'EXCELLENT' ? 'success' : 'warning'
                  }
                  size="small"
                />
              </Box>
          ...
          <Grid size={{ xs: 12, sm: 6, md: 3 }}>
            ...
            >
              ...
              <Typography
                ...
                color="success.main"
              >
                {drStatus.rpoHours < 900
                  ? `${drStatus.rpoHours} hrs`
                  : 'No Backup'}
              </Typography>
              <Typography variant="caption" color="text.secondary">
                Target: &lt; 24 hrs data loss window
              </Typography>
            </Paper>
          </Grid>
          <Grid size={{ xs: 12, sm: 6, md: 3 }}>
          ...
          <Grid size={{ xs: 12, sm: 6, md: 3 }}>
            ...
            >
              ...
              >
                <UsbIcon
                  color={
                    drStatus.components.raspberryPiHardware
              ...
              <Typography variant="caption" color="text.secondary">
                {drStatus.components.raspberryPiHardware.isRaspberryPiDetected
      ...
      >
        ...
        >
          ...
          <Tab
            icon={<SecurityIcon />}
            iconPosition="start"
            label="Disaster Recovery Readiness"
            sx={{ fontWeight: 700 }}
          />
        </Tabs>
            ...
            {backups.length === 0 ? (
              <Box sx={{ textAlign: 'center', py: 5 }}>
                ...
                <Typography variant="h6" color="text.secondary">
                  No infrastructure backups created yet
                </Typography>
                <Button
                  variant="contained"
                  sx={{ mt: 2 }}
                    ...
                    <TableRow key={b.id}>
                      ...
                      <TableCell>
                        <Typography variant="body2" sx={{ fontWeight: 600 }}>
                          {formatBytes(b.fileSizeCompressed)}
                        </Typography>
                        <Typography variant="caption" color="text.secondary">
                          Orig: {formatBytes(b.fileSizeOriginal)}
                        </Typography>
                      </TableCell>
                      <TableCell>
                        <Typography variant="body2" sx={{ fontWeight: 700 }}>
                          {b.compressionRatio
                            ? `${Math.round((1 - b.compressionRatio) * 100)}% saved`
                            : '1:1'}
                        </Typography>
                      </TableCell>
                      <TableCell>
                        {b.durationMs ? `${b.durationMs} ms` : 'N/A'}
                      </TableCell>
                      <TableCell>
                        ...
                        >
                          <Tooltip title="Verify Integrity & Checksum">
                            <IconButton
                              ...
                              color="info"
                              onClick={() => {
                                void handleVerifyBackup(b.id, b.name);
                              }}
                            >
                          ...
                          <Tooltip title="Restore Platform to this Backup">
                            <Button
                              ...
                              onClick={() => {
                                void handleOpenRestorePreview(b);
                              }}
                            >
                              Restore
                            </Button>
                          ...
                          <Tooltip title="Delete Backup Archive">
                            <IconButton
                              ...
                              color="error"
                              onClick={() => {
                                void handleDeleteBackup(b.id, b.name);
                              }}
                            >
        ...
        {activeTab === 1 && (
          <Box sx={{ p: 2 }}>
            ...
            {restoreHistory.length === 0 ? (
              <Typography variant="body2" color="text.secondary">
                No restore operations executed yet
              </Typography>
            ) : (
              <Table size="small">
                <TableHead>
          ...
          <Box sx={{ p: 2 }}>
            ...
            >
              <Typography variant="subtitle1" sx={{ fontWeight: 800 }}>
                Automated Backup Schedule Policies
              </Typography>
              <Button
                variant="contained"
                size="small"
                ...
                <Grid size={{ xs: 12, md: 6 }} key={idx}>
                  ...
                  >
                    ...
                    >
                      ...
                      <Chip
                        label={`Risk: ${sc.riskLevel}`}
                        color={sc.riskLevel === 'LOW' ? 'success' : 'warning'}
                        size="small"
                      />
                    </Box>
                    <Typography
      ...
      >
        <DialogTitle sx={{ fontWeight: 800 }}>
          Create Manual / Emergency Backup
        </DialogTitle>
        <DialogContent dividers>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 1 }}>
            <TextField
              ...
              onChange={(e) =>
                setNewBackupDto({ ...newBackupDto, name: e.target.value })
              }
            />
            <FormControl fullWidth>
              ...
              <Select
                ...
                label="Backup Type"
                onChange={(e) =>
                  setNewBackupDto({ ...newBackupDto, type: e.target.value })
                }
              >
          ...
          {selectedBackupForRestore && (
            <Box sx={{ py: 1 }}>
              <Alert severity="warning" sx={{ mb: 2 }}>
                Restoring will update database records and physical
                configurations to the state captured in '
              ...
              {restorePreview ? (
                ...
                >
                  <Typography variant="subtitle2" sx={{ fontWeight: 800 }}>
                    RESTORE PREVIEW METRICS:
                  </Typography>
                  <Typography variant="body2">
                    Archive Entries: {restorePreview.totalFiles} files
                  </Typography>
                  <Typography variant="body2">
                    Database Tables Affected:{' '}
                    {restorePreview.dbTablesAffected.join(', ')}
                  </Typography>
                  <Typography variant="body2">
                    Estimated Time: {restorePreview.estimatedTimeMs} ms
                  </Typography>
                </Paper>
              ) : (
        ...
        <DialogContent dividers>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 1 }}>
            <TextField
              ...
              onChange={(e) =>
                setScheduleDto({ ...scheduleDto, name: e.target.value })
              }
            />
            <FormControl fullWidth>
              ...
              <Select
                ...
                label="Frequency"
                onChange={(e) =>
                  setScheduleDto({ ...scheduleDto, frequency: e.target.value })
                }
              >
