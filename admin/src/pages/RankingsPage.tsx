...
export const RankingsPage: React.FC = () => {
  ...
  const handleCreateSnapshot = async () => {
    try {
      ...
      addToast('success', `Created historical snapshot for ${snapshotCategory}`);
      setSnapshotDialogOpen(false);
      fetchRankings();
    } catch (err: any) {
      addToast('error', err?.response?.data?.message || 'Failed to create snapshot');
    }
  };
  ...
  const userColumns: Column<LeaderboardItem>[] = [
    ...
    { id: 'country', label: 'Country', render: (row) => row.country || 'GLOBAL' },
    {
      id: 'coins',
      label: 'Coins',
  ...
  const hostColumns: Column<LeaderboardItem>[] = [
    {
      ...
      label: 'Rank',
      render: (row) => (
        <Chip label={`#${row.rank}`} color={row.rank <= 3 ? 'warning' : 'default'} size="small" />
      ),
    },
    { id: 'hostName', label: 'Host', render: (row) => row.hostName || row.id },
    { id: 'level', label: 'Level', render: (row) => `Lvl ${row.level || 1}` },
    { id: 'totalAudience', label: 'Total Audience', render: (row) => (row.totalAudience || 0).toLocaleString() },
    { id: 'giftsEarned', label: 'Gifts Earned', render: (row) => (row.giftsEarned || 0).toLocaleString() },
    { id: 'engagementScore', label: 'Engagement Score', render: (row) => `${row.engagementScore || 85}%` },
    { id: 'country', label: 'Country', render: (row) => row.country || 'GLOBAL' },
  ];
    ...
    <Box>
      <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            Phase 27: Multi-category leaderboards, rolling trending windows, Redis caching, BullMQ background jobs, and historical snapshots
          </Typography>
        </Box>
      ...
      <Box sx={{ borderBottom: 1, borderColor: 'divider', mb: 3 }}>
        <Tabs value={activeTab} onChange={(_, val) => setActiveTab(val)} variant="scrollable" scrollButtons="auto">
          <Tab label="User Rankings" icon={<PeopleIcon />} iconPosition="start" />
          <Tab label="Host Rankings" icon={<RecordVoiceOverIcon />} iconPosition="start" />
          <Tab label="Agency Rankings" icon={<BusinessIcon />} iconPosition="start" />
          <Tab label="Club Rankings" icon={<GroupsIcon />} iconPosition="start" />
          <Tab label="Room Rankings" icon={<MeetingRoomIcon />} iconPosition="start" />
          <Tab label="VIP Rankings" icon={<WorkspacePremiumIcon />} iconPosition="start" />
          <Tab label="Creator Rankings" icon={<AutoAwesomeIcon />} iconPosition="start" />
          <Tab label="Trending" icon={<TrendingUpIcon />} iconPosition="start" />
          <Tab label="Historical Snapshots" icon={<HistoryIcon />} iconPosition="start" />
          <Tab label="Cache & Engine Status" icon={<MemoryIcon />} iconPosition="start" />
        </Tabs>
      </Box>
      ...
      {activeTab === 0 && (
        <Box>
          ...
          <DataTable columns={userColumns} rows={userRankings} loading={loading} />
        </Box>
      )}
      ...
      {activeTab === 1 && (
        <Box>
          ...
          <DataTable columns={hostColumns} rows={hostRankings} loading={loading} />
        </Box>
      )}
        ...
        <Box>
          ...
          <DataTable
            columns={[
              ...
              { id: 'name', label: 'Agency Name', render: (r) => r.name || r.id },
              { id: 'totalRevenue', label: 'Revenue ($)', render: (r) => `$${(r.totalRevenue || 0).toLocaleString()}` },
              { id: 'memberCount', label: 'Members', render: (r) => r.memberCount || 0 },
              { id: 'activeHostCount', label: 'Active Hosts', render: (r) => r.activeHostCount || 0 },
              { id: 'country', label: 'Country', render: (r) => r.country || 'GLOBAL' },
            ]}
            rows={agencyRankings}
            loading={loading}
          />
        ...
        <Box>
          ...
          <DataTable
            columns={[
              ...
              { id: 'category', label: 'Category', render: (r) => r.category || 'General' },
              { id: 'memberCount', label: 'Members', render: (r) => r.memberCount || 0 },
              { id: 'weeklyActivityScore', label: 'Activity Score', render: (r) => r.weeklyActivityScore || 0 },
            ]}
            rows={clubRankings}
            loading={loading}
          />
      ...
      {activeTab === 4 && (
        <Box>
          ...
          <DataTable
            columns={[
              { id: 'rank', label: 'Rank', render: (r) => `#${r.rank}` },
              { id: 'title', label: 'Room Title', render: (r) => r.title || r.id },
              { id: 'listenerCount', label: 'Active Listeners', render: (r) => r.listenerCount || 0 },
              { id: 'popularityScore', label: 'Popularity Score', render: (r) => r.popularityScore || 0 },
              { id: 'giftActivity', label: 'Gifts Activity', render: (r) => r.giftActivity || 0 },
            ]}
            rows={roomRankings}
            loading={loading}
          />
        ...
        <Box>
          ...
          <DataTable
            columns={[
              ...
              { id: 'tierName', label: 'Tier', render: (r) => r.tierName || 'VIP' },
              { id: 'level', label: 'VIP Level', render: (r) => `Lvl ${r.level || 1}` },
              { id: 'experience', label: 'XP Points', render: (r) => (r.experience || 0).toLocaleString() },
              { id: 'lifetimeSpending', label: 'Spending ($)', render: (r) => `$${(r.lifetimeSpending || 0).toFixed(2)}` },
            ]}
            rows={vipRankings}
            loading={loading}
          />
      ...
      {activeTab === 6 && (
        <Box>
          ...
          <DataTable
            columns={[
              { id: 'rank', label: 'Rank', render: (r) => `#${r.rank}` },
              { id: 'username', label: 'Creator', render: (r) => r.displayName || r.username || r.id },
              { id: 'creatorRevenue', label: 'Revenue ($)', render: (r) => `$${(r.creatorRevenue || 0).toFixed(2)}` },
              { id: 'followersCount', label: 'Followers', render: (r) => (r.followersCount || 0).toLocaleString() },
            ]}
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <Card variant="outlined">
                  <CardContent>
                    ...
                    {trendingData.fastestRisingUsers?.slice(0, 5).map((u: any, i: number) => (
                      <Box key={i} sx={{ display: 'flex', justifyContent: 'space-between', my: 1 }}>
                        <Typography variant="body2">#{i + 1} {u.displayName || u.username}</Typography>
                        <Chip label={u.growthRate || '+120%'} color="error" size="small" />
                      </Box>
                    ))}
                  </CardContent>
                </Card>
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <Card variant="outlined">
                  <CardContent>
                    ...
                    {trendingData.trendingHosts?.slice(0, 5).map((h: any, i: number) => (
                      <Box key={i} sx={{ display: 'flex', justifyContent: 'space-between', my: 1 }}>
                        <Typography variant="body2">#{i + 1} {h.stageName || h.id}</Typography>
                        <Chip label={h.audienceVelocity || '+400/hr'} color="primary" size="small" />
                      </Box>
                    ))}
                  </CardContent>
                </Card>
        ...
        <Box>
          ...
          <DataTable
            columns={[
              ...
              { id: 'timeframe', label: 'Timeframe', render: (s) => s.timeframe },
              { id: 'periodIdentifier', label: 'Period', render: (s) => s.periodIdentifier },
              { id: 'totalCount', label: 'Entities Ranked', render: (s) => s.totalCount },
              { id: 'createdAt', label: 'Created At', render: (s) => new Date(s.createdAt).toLocaleString() },
            ]}
            rows={snapshots}
            loading={loading}
          />
        ...
        <Box>
          ...
          <Card variant="outlined" sx={{ p: 2 }}>
            <Typography variant="body2" sx={{ mb: 1 }}>
              Last Refreshed At: <strong>{new Date(cacheStatus.lastRefreshedAt).toLocaleString()}</strong>
            </Typography>
            <Typography variant="body2" sx={{ mb: 2 }}>
              Total Active Cached Keys: <strong>{cacheStatus.totalCached}</strong>
            </Typography>
