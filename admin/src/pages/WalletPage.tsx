...
export const WalletPage: React.FC = () => {
  ...
  const payoutColumns: Column<AdminCreatorPayout>[] = [
    ...
    { id: 'payoutAmount', label: 'USD', render: (row) => `$${Number(row.payoutAmount).toFixed(2)}` },
    ...
    { id: 'status', label: 'Status', render: (row) => <StatusBadge status={row.status.toLowerCase()} /> },
    {
      ...
      render: (row) => (
        <Stack direction="row" spacing={1}>
          {row.status === 'PENDING' && (
            <>
              <Button size="small" color="success" startIcon={<CheckCircleIcon />} onClick={() => actOnPayout(row, 'approve')}>Approve</Button>
              <Button size="small" color="error" startIcon={<CancelIcon />} onClick={() => actOnPayout(row, 'reject')}>Reject</Button>
            </>
          )}
          {row.status === 'APPROVED' && (
            <Button size="small" color="primary" startIcon={<PaymentsIcon />} onClick={() => actOnPayout(row, 'process')}>Settle</Button>
          )}
        </Stack>
      ),
    },
  ...
  return (
    <Box>
      <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', gap: 2, flexWrap: 'wrap' }}>
        <Box>
          <Typography variant="h4" sx={{ fontWeight: 800 }}>Wallet & Economy Management</Typography>
          <Typography variant="body2" color="text.secondary">
            Live treasury metrics, immutable wallet ledger and Creator payout review.
          </Typography>
        </Box>
        <Button startIcon={<RefreshIcon />} variant="outlined" onClick={fetchData} disabled={loading}>Refresh</Button>
      </Box>
      ...
      {loading && !overview ? (
        ...
        <>
          ...
          {tab === 0 && <DataTable columns={transactionColumns} rows={transactions} />}
