...
export const StorePage: React.FC = () => {
  ...
  const fetchCatalog = async () => {
    ...
    try {
      ...
      addToast(
        err.response?.data?.message || 'Failed to fetch store catalog',
        'error',
      );
    } finally {
      setLoading(false);
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      addToast('error', 'Failed to fetch store analytics');
    }
  };
  ...
  const handleSaveItem = async () => {
    if (!editingItem?.name || !editingItem?.iconUrl || !editingItem?.assetUrl) {
      addToast(
        'warning',
        'Please fill in required fields (Name, Icon URL, Asset URL)',
      );
      return;
    }
    ...
    try {
      if (editingItem.id) {
        ...
        addToast('success', 'Store item updated successfully');
      } else {
        await adminStoreService.createItem(editingItem);
        addToast('success', 'Store item created successfully');
      }
      ...
      addToast(
        'error',
        err.response?.data?.message || 'Failed to save store item',
      );
    }
  };

  const handleDeleteItem = async (id: string) => {
    if (!window.confirm('Are you sure you want to deactivate this store item?'))
      return;
    try {
      await adminStoreService.deleteItem(id);
      addToast('info', 'Store item deactivated');
      fetchCatalog();
    } catch (err: any) {
      addToast('error', 'Failed to deactivate store item');
    }
  };

  const handleGrantItem = async () => {
    if (!grantUserId || !grantItemId) {
      addToast('warning', 'Please provide User ID and select an Item');
      return;
    }
    ...
    try {
      ...
      addToast('success', 'Decor item granted to user successfully');
      setGrantModalOpen(false);
      setGrantUserId('');
      setGrantReason('');
    } catch (err: any) {
      addToast('error', err.response?.data?.message || 'Failed to grant item');
    }
  };
  ...
  return (
    <Box sx={{ p: 3 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          ...
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              Avatar Frames, Chat Bubbles, Entrance Effects, Room Themes,
              Vehicles & Mall Management
            </Typography>
          </Box>
        </Box>
          ...
          <Grid size={{ xs: 12, sm: 6, md: 3 }}>
            <Card sx={{ bgcolor: 'background.paper', borderRadius: 2 }}>
              <CardContent>
                ...
                <Typography
                  variant="h4"
                  sx={{ fontWeight: 800, mt: 1, color: 'warning.main' }}
                >
          ...
          <Grid size={{ xs: 12, sm: 6, md: 3 }}>
            <Card sx={{ bgcolor: 'background.paper', borderRadius: 2 }}>
              <CardContent>
                ...
                <Typography
                  variant="h4"
                  sx={{ fontWeight: 800, mt: 1, color: 'secondary.main' }}
                >
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow>
                <TableCell colSpan={7} align="center" sx={{ py: 4 }}>
                  <Typography variant="body2" color="text.secondary">
                    No store items found in catalog. Create one using 'New Store
                    Item'!
                  </Typography>
                </TableCell>
              </TableRow>
              ...
              items.map((item) => (
                <TableRow key={item.id} hover>
                  ...
                  <TableCell>
                    {item.isVipExclusive ? (
                      <Chip
                        label={`VIP Lvl ${item.minVipLevel}+`}
                        size="small"
                        color="secondary"
                        variant="outlined"
                      />
            ...
            <Grid container spacing={2} sx={{ pt: 1 }}>
              ...
              <Grid size={{ xs: 12, sm: 6 }}>
                <TextField
                  ...
                  onChange={(e) =>
                    setEditingItem({ ...editingItem, iconUrl: e.target.value })
                  }
                />
              </Grid>
              <Grid size={{ xs: 12, sm: 6 }}>
                <TextField
                  ...
                  onChange={(e) =>
                    setEditingItem({ ...editingItem, assetUrl: e.target.value })
                  }
                />
              </Grid>
              <Grid size={{ xs: 12, sm: 6 }}>
                  ...
                  control={
                    <Switch
                      ...
                      onChange={(e) =>
                        setEditingItem({
                          ...editingItem,
                          isVipExclusive: e.target.checked,
                        })
                      }
                    />
                  }
      ...
      >
        <DialogTitle sx={{ fontWeight: 800 }}>
          Grant Store Decor Item to User
        </DialogTitle>
        <DialogContent dividers>
