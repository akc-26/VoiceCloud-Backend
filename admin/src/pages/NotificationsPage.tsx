...
export const NotificationsPage: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const result = await notificationsAdminService.getDeliveryLog({
        page: 1,
        limit: 100,
      });
      setLogs(result.data || []);
      ...
      addToast(
        'error',
        error.message || 'Failed to load notification delivery log',
      );
    } finally {
      setLoading(false);
    }
  ...
  const columns: Column<AdminNotificationRecord>[] = [
    ...
    { id: 'type', label: 'Type' },
    {
      ...
      label: 'Delivery',
      render: (row) => (
        <StatusBadge status={row.deliveryStatus.toLowerCase()} />
      ),
    },
      ...
      >
        <Box>
          <Typography variant="h4" sx={{ fontWeight: 800 }}>
            Notification Delivery Console
          </Typography>
          <Typography variant="body2" color="text.secondary">
            Persist notifications once and monitor BullMQ delivery state and
            retry attempts.
          </Typography>
        </Box>
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            variant="contained"
            onClick={send}
            disabled={!form.userId || !form.title || !form.message}
          >
