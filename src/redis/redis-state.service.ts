...
export class RedisStateService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    try {
      if (typeof this.redisClient.duplicate === 'function') {
        ...
        this.logger.log(
          `Redis Pub/Sub subscribed to channel ${REDIS_KEYS.GLOBAL_PUBSUB_CHANNEL}`,
        );
      }
    } catch (err: any) {
  ...
  async publishEvent(
    ...
    try {
      ...
      await this.redisClient.publish(
        REDIS_KEYS.GLOBAL_PUBSUB_CHANNEL,
        JSON.stringify(message),
      );
    } catch (err: any) {
      this.logger.warn(
        `Failed to publish Redis event ${event}: ${err.message}`,
      );
    }
  }
  ...
  private handleIncomingPubSubMessage(messageStr: string) {
    try {
      ...
      this.logger.error(
        `Error parsing incoming PubSub message: ${err.message}`,
      );
    }
  }
  ...
  async setSpeaker(
    roomId: string,
    speaker: RedisStageSpeaker,
  ): Promise<RedisStageSpeaker[]> {
    try {
  ...
  async enqueueUser(
    ...
    try {
      ...
      const position =
        updatedQueue.findIndex((q) => q.userId === queueUser.userId) + 1;
    ...
    try {
      ...
      if (newQueue.length > 0) {
        await this.redisClient.expire(
          queueKey,
          DEFAULT_TTLS.ROOM_STATE_SECONDS,
        );
      }
  ...
  async getInvitation(
    roomId: string,
    targetUserId: string,
  ): Promise<RedisPendingInvitation | null> {
    try {
      const key = REDIS_KEYS.ROOM_INVITATIONS(roomId);
  ...
  async cleanupRoomState(roomId: string): Promise<void> {
    try {
      ...
      pipeline.del(REDIS_KEYS.ROOM_LAYOUT(roomId));
      pipeline.del(REDIS_KEYS.ROOM_PINNED_MESSAGE(roomId));
      await pipeline.exec();
    } catch (err: any) {
  ...
  // --- Stage 2: Stage Layout & Pinned Message ---
  async setRoomLayout(roomId: string, layout: string): Promise<void> {
    try {
      const key = REDIS_KEYS.ROOM_LAYOUT(roomId);
      await this.redisClient.set(key, layout, 'EX', DEFAULT_TTLS.ROOM_STATE_SECONDS);
    } catch (err: any) {
      this.logger.warn(`Redis setRoomLayout error: ${err.message}`);
    }
  }

  async getRoomLayout(roomId: string): Promise<string | null> {
    try {
      const key = REDIS_KEYS.ROOM_LAYOUT(roomId);
      return await this.redisClient.get(key);
    } catch (err: any) {
      this.logger.warn(`Redis getRoomLayout error: ${err.message}`);
      return null;
    }
  }

  async setPinnedMessage(roomId: string, messageData: any): Promise<void> {
    try {
      const key = REDIS_KEYS.ROOM_PINNED_MESSAGE(roomId);
      await this.redisClient.set(key, JSON.stringify(messageData), 'EX', DEFAULT_TTLS.ROOM_STATE_SECONDS);
    } catch (err: any) {
      this.logger.warn(`Redis setPinnedMessage error: ${err.message}`);
    }
  }
  ...
  async getPinnedMessage(roomId: string): Promise<any | null> {
    try {
      ...
      this.logger.warn(`Redis getPinnedMessage error: ${err.message}`);
      return null;
    }
  }
}
