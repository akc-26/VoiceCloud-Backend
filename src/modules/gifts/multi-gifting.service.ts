...
export class MultiGiftingService {
  private readonly REVENUE_SHARE_PERCENT = 0.70; // 70% to creator/speaker in diamonds
  ...
  async sendMultiRecipientGift(senderId: string, dto: SendMultiRecipientGiftDto) {
    const { targetUserIds, roomId, giftId, quantity = 1, pricePerUnit = 100 } = dto;

    if (!targetUserIds || targetUserIds.length === 0) {
      throw new BadRequestException('At least one recipient target user ID must be specified');
    }
    ...
    const senderBalanceRaw = await this.redisService.get(`wallet:${senderId}:coins`);
    const senderCoins = senderBalanceRaw ? parseInt(senderBalanceRaw, 10) : 10000; // Default simulated balance if new
    ...
    await this.redisService.set(`wallet:${senderId}:coins`, newSenderCoins.toString());

    // Calculate diamond allocation per recipient
    const totalDiamondsGenerated = Math.floor(totalCoinsRequired * this.REVENUE_SHARE_PERCENT);
    const diamondsPerRecipient = Math.floor(totalDiamondsGenerated / recipientCount);

    const recipientAllocations: { userId: string; diamondsEarned: number }[] = [];

    for (const recipientId of targetUserIds) {
      const currentDiamondsRaw = await this.redisService.get(`wallet:${recipientId}:diamonds`);
      const currentDiamonds = currentDiamondsRaw ? parseInt(currentDiamondsRaw, 10) : 0;
      const newDiamonds = currentDiamonds + diamondsPerRecipient;

      await this.redisService.set(`wallet:${recipientId}:diamonds`, newDiamonds.toString());
      recipientAllocations.push({ userId: recipientId, diamondsEarned: diamondsPerRecipient });
    }
    ...
    await this.redisService.set(`tx:multigift:${txId}`, JSON.stringify(txData), 86400 * 7);
    ...
    this.eventsGateway.server.to(`room:${roomId}`).emit('room_multi_gift_blast', broadcastPayload);
