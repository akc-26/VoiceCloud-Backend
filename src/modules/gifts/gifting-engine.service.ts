...
export class GiftingEngineService {
  ...
  async sendGift(senderId: string, dto: SendGiftDto) {
    const { giftId, context = 'room', roomId, quantity = 1, comboStep = 1 } = dto;
    ...
    if (!gift.isActive || gift.isArchived) {
      throw new BadRequestException(`Gift '${gift.name}' is currently unavailable`);
    }
    ...
    const senderCoinsRaw = await this.redisService.get(`wallet:${senderId}:coins`);
    ...
    for (const rId of receivers) {
      ...
      const receiverDiamondsRaw = await this.redisService.get(`wallet:${rId}:diamonds`);
      const currentDiamonds = receiverDiamondsRaw ? parseInt(receiverDiamondsRaw, 10) : 0;
    ...
    const isFullscreen = gift.type === 'svga' || gift.type === 'video' || gift.coinPrice >= 1000;
    ...
    if (prevCombo === 0) {
      ...
      this.eventsGateway.broadcastComboUpdated(
        { senderId, giftId: gift.id, comboCount: newComboCount, multiplier: comboMultiplier, roomId },
        roomId,
      );
    }
  ...
  async cancelQueuedGift(userId: string, queueId: string): Promise<{ success: boolean; message: string }> {
    ...
    if (item.status !== 'queued') {
      throw new BadRequestException(`Cannot cancel gift in status '${item.status}'`);
    }
  ...
  async getGiftHistory(
    ...
    const where = role === 'sender' ? { senderId: userId } : { receiverId: userId };
