...
export class GiftsService {
  ...
  async createDynamicGift(dto: CreateDynamicGiftDto): Promise<Gift> {
    const gift = this.giftRepository.create({
      ...
      agencyEarningsPercentage: dto.agencyEarningsPercentage ?? 10.0,
      ...
      isAgencyExclusive: dto.isAgencyExclusive ?? false,
  ...
  async updateDynamicGift(
    ...
    if (dto.agencyEarningsPercentage !== undefined)
      gift.agencyEarningsPercentage = dto.agencyEarningsPercentage;
    ...
    if (dto.isAgencyExclusive !== undefined)
      gift.isAgencyExclusive = dto.isAgencyExclusive;
  ...
  async getCatalog(
    ...
    return gifts.filter((gift) => {
      ...
      // Type filter
      if (options.type && gift.type.toLowerCase() !== options.type.toLowerCase()) {
        return false;
      }
  ...
  async getFeaturedGifts(): Promise<Gift[]> {
    ...
    return gifts
      .filter((g) => g.rarity === 'epic' || g.rarity === 'legendary' || g.isLimitedEdition || g.isSeasonal)
      .slice(0, 10);
  }
  ...
  async getCategories(): Promise<GiftCategory[]> {
    ...
    return defaultNames.map((name, idx) => ({
      ...
      isAgencyExclusive: name.includes('Agency'),
      createdAt: new Date(),
      updatedAt: new Date(),
    })) as GiftCategory[];
  }
  ...
  async updateCategory(
    ...
    if (dto.isAgencyExclusive !== undefined)
      cat.isAgencyExclusive = dto.isAgencyExclusive;
