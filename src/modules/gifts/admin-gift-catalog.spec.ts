...
describe('Admin gift catalog visibility', () => {
  it('returns the complete persisted gift catalog without public availability filtering', async () => {
    const rows = [
      ...
      { id: 'gift-disabled', isActive: false, isArchived: false, isHidden: false },
      { id: 'gift-archived', isActive: false, isArchived: true, isHidden: true },
    ];
    ...
    const categoryRepository = {
      find: jest.fn().mockResolvedValue([
        { id: 'category-disabled', name: 'Legacy', isActive: false, sortOrder: 2 },
        { id: 'category-active', name: 'Popular', isActive: true, sortOrder: 1 },
      ]),
    } as any;
