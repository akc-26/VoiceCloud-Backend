...
@ApiTags('Phase 18 Gamified & Multi Gifting')
@Controller('gifts')
...
export class GiftsPhase18Controller {
  ...
  @ApiOperation({ summary: 'Send gift to multiple target recipients simultaneously in a room' })
  @ApiResponse({ status: 200, description: 'Multi-recipient gift sent successfully' })
  @ApiResponse({ status: 400, description: 'Insufficient coin balance or invalid target recipients' })
  async sendMultiGift(
  ...
  @Post('lucky-box/open')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Open mystery lucky box for gamified rewards & coin cashback' })
