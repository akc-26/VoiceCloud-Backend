...
export class LuckyBoxService {
  ...
  async openLuckyBox(userId: string, dto: OpenLuckyBoxDto) {
    ...
    await this.redisService.set(`wallet:${userId}:coins`, remainingCoins.toString());
    ...
    await this.redisService.set(`wallet:${userId}:coins`, finalCoins.toString());
    ...
    if (roomId && hasJackpot) {
      this.eventsGateway.server.to(`room:${roomId}`).emit('room_jackpot_announcement', {
        userId,
        roomId,
        tier,
        rewards,
