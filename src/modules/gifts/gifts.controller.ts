...
import { SendGiftDto, SendComboDto, SendMultiGiftPhase22Dto } from './dto/send-gift-phase22.dto';
...
export class GiftsController {
  ...
  @Get('catalog')
  @ApiOperation({
    summary: 'List available gift catalog with regional, category, and seasonal filters',
  })
  @ApiQuery({ name: 'countryCode', required: false, description: 'User country code (e.g. US, IN)' })
  @ApiQuery({ name: 'seasonTag', required: false, description: 'Season tag (e.g. summer_2026)' })
  @ApiQuery({ name: 'category', required: false, description: 'Filter by category name' })
  @ApiQuery({ name: 'type', required: false, description: 'Filter by gift type (static, animated, svga, etc.)' })
  ...
  async getCatalog(
    ...
    return this.giftsService.getCatalog(countryCode, seasonTag, { category, type });
  }
  ...
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Create new gift item with metadata and availability rules (Admin)' })
  ...
  @ApiBearerAuth()
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Send gift to single or multiple receivers in room/chat/event' })
  ...
  @ApiQuery({ name: 'timeframe', required: false, enum: ['hourly', 'daily', 'weekly', 'monthly'] })
  @ApiResponse({ status: 200, description: 'Top gifts list' })
  async getTopGifts(@Query('timeframe') timeframe?: 'hourly' | 'daily' | 'weekly' | 'monthly') {
  ...
  async getTopSenders(@Query('timeframe') timeframe?: 'hourly' | 'daily' | 'weekly' | 'monthly') {
  ...
  async getTopReceivers(@Query('timeframe') timeframe?: 'hourly' | 'daily' | 'weekly' | 'monthly') {
    return this.giftAnalyticsService.getTopReceivers(timeframe || 'daily');
  }

  @Get('analytics/revenue')
  @ApiOperation({ summary: 'Get gifting revenue, creator payouts, and agency share summary' })
  @ApiQuery({ name: 'timeframe', required: false, enum: ['hourly', 'daily', 'weekly', 'monthly'] })
  @ApiResponse({ status: 200, description: 'Revenue metrics summary' })
  async getGiftRevenue(@Query('timeframe') timeframe?: 'hourly' | 'daily' | 'weekly' | 'monthly') {
  ...
  async getGiftTrends(@Query('timeframe') timeframe?: 'hourly' | 'daily' | 'weekly' | 'monthly') {
  ...
  @Post(':id/animation')
  @ApiOperation({ summary: 'Upload gift animation file (SVGA, Lottie, GIF, MP4, WebM)' })
  @ApiConsumes('multipart/form-data')
  @ApiBody({
    schema: {
      type: 'object',
  ...
  @ApiResponse({ status: 201, description: 'Gift animation uploaded successfully' })
  @UseInterceptors(FileInterceptor('file'))
  ...
  @ApiResponse({ status: 201, description: 'Gift preview uploaded successfully' })
  @UseInterceptors(FileInterceptor('file'))
