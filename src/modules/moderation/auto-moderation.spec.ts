...
describe('Phase18 AutoModeration & Device Security', () => {
  ...
  beforeEach(async () => {
    ...
    deviceSecurityService = module.get<DeviceSecurityService>(DeviceSecurityService);
  });

  describe('AutoModerationService', () => {
    it('should evaluate clean content as safe', () => {
      const result = autoModerationService.analyzeContent({ text: 'Hello everyone in the voice room!' });
    ...
    it('should flag spam/phishing text with high toxicity score', () => {
      const result = autoModerationService.analyzeContent({ text: 'Claim free-coins-click-here now!' });
  ...
  describe('DeviceSecurityService', () => {
    ...
    it('should throw BadRequestException when registering a banned device', async () => {
      ...
      await expect(
        deviceSecurityService.registerDevice('user-1', { deviceId: 'fp-banned' }),
