...
export class Phase18SecurityController {
  ...
  @ApiOperation({
    summary:
      'Analyze content text against automated safety rules & toxicity filters',
  })
  @ApiResponse({
    status: 200,
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Register client device fingerprint and anti-fraud association',
  })
  ...
  @ApiOperation({
    summary: 'Check if a device fingerprint ID is currently banned',
  })
  @ApiParam({ name: 'deviceId', description: 'Device fingerprint ID string' })
  @ApiResponse({
    status: 200,
    description: 'Device ban status checked successfully',
  })
