...
export class CreatorService {
   ...
   */
  async getStreamCredentials(creatorId: string) {
    let key = this.streamKeys.get(creatorId);
    if (!key) {
      const cleanId = (creatorId || 'default').replace(/-/g, '').substring(0, 16);
      key = `live_vc_sk_${cleanId}`;
      this.streamKeys.set(creatorId, key);
    }
    return {
   ...
   */
  async regenerateStreamKey(creatorId: string) {
    const randomSuffix = Math.random().toString(36).substring(2, 10) + Math.random().toString(36).substring(2, 10);
    const newKey = `live_vc_sk_${randomSuffix}`;
    this.streamKeys.set(creatorId, newKey);
    return {
      streamKey: newKey,
      rtmpUrl: 'rtmps://live.voicecloud.app:443/live',
      audioBitrate: '324',
