...
describe('Phase 2D Creator Economy Business APIs', () => {
  ...
  describe('CreatorController Delegation', () => {
    ...
    it('should return stream credentials and allow key regeneration', async () => {
      const creds = await service.getStreamCredentials('user-1');
      expect(creds.rtmpUrl).toBe('rtmps://live.voicecloud.app:443/live');
      expect(creds.streamKey).toContain('live_vc_sk_');
      expect(creds.audioBitrate).toBe('324');

      const regen = await service.regenerateStreamKey('user-1');
      expect(regen.streamKey).toContain('live_vc_sk_');
      expect(regen.streamKey).not.toBe(creds.streamKey);

      const controllerGet = await controller.getStreamCredentials('user-1');
      expect(controllerGet.streamKey).toBe(regen.streamKey);

      const controllerRegen = await controller.regenerateStreamKey('user-1');
