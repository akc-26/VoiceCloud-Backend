...
describe('Phase 2D Creator Economy Business APIs', () => {
  ...
  describe('CreatorService - Plan Management', () => {
    ...
    it('should update plan if caller is owner', async () => {
      ...
      const updated = await service.updatePlan('creator-1', 'plan-1', {
        title: 'New Title',
      });
    ...
    it('should update plan benefits if caller is owner', async () => {
      const existingPlan = {
        id: 'plan-1',
        creatorId: 'creator-1',
      ...
      const updated = await service.updatePlanBenefits('creator-1', 'plan-1', [
        'New Benefit 1',
        'New Benefit 2',
      ]);
    ...
    it('should throw NotFoundException on public plans if creator does not exist', async () => {
      ...
      await expect(
        service.getPublicCreatorPlans('non-existent'),
      ).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('CreatorService - Subscriptions', () => {
    it('should throw BadRequestException if user subscribes to self', async () => {
      await expect(
        service.subscribeToCreator('creator-1', 'creator-1', {
          planId: 'plan-1',
        }),
    ...
    it('should throw NotFoundException if plan for subscription is missing', async () => {
      ...
      await expect(
        service.subscribeToCreator('user-1', 'creator-1', {
          planId: 'plan-invalid',
        }),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('should create subscription intent record successfully', async () => {
      ...
      const res = await service.subscribeToCreator('user-1', 'creator-1', {
        planId: 'plan-1',
      });
  ...
  describe('CreatorController Delegation', () => {
    it('should delegate getDashboard to service', async () => {
      const spy = jest
        .spyOn(service, 'getCreatorDashboard')
        .mockResolvedValue({ subscriberCount: 5 } as any);
    ...
    it('should delegate createPlan to service', async () => {
      ...
      const spy = jest
        .spyOn(service, 'createPlan')
        .mockResolvedValue({ id: 'plan-1' } as any);
      const res = await controller.createPlan('user-1', dto);
    ...
    it('should delegate getMyPlans to service', async () => {
      const spy = jest
        .spyOn(service, 'getCreatorPlans')
        .mockResolvedValue({ total: 1 } as any);
    ...
    it('should delegate getPublicPlans to service', async () => {
      const spy = jest
        .spyOn(service, 'getPublicCreatorPlans')
        .mockResolvedValue([]);
    ...
    it('should delegate updatePlanBenefits to service', async () => {
      const spy = jest
        .spyOn(service, 'updatePlanBenefits')
        .mockResolvedValue({ id: 'plan-1', benefits: ['Benefit A'] } as any);
      const res = await controller.updatePlanBenefits('user-1', 'plan-1', {
        benefits: ['Benefit A'],
      });
      expect(spy).toHaveBeenCalledWith('user-1', 'plan-1', ['Benefit A']);
      expect(res).toEqual({ id: 'plan-1', benefits: ['Benefit A'] });
    });

    it('should delegate deletePlan to service', async () => {
      const spy = jest.spyOn(service, 'deletePlan').mockResolvedValue({
        id: 'plan-1',
    ...
    it('should delegate subscribe to service', async () => {
      ...
      const spy = jest
        .spyOn(service, 'subscribeToCreator')
    ...
    it('should delegate getMySubscriptions to service', async () => {
      const spy = jest
        .spyOn(service, 'getUserSubscriptions')
    ...
    it('should delegate getSubscribers to service', async () => {
      const spy = jest
        .spyOn(service, 'getCreatorSubscribers')
        .mockResolvedValue({ total: 0 } as any);
    ...
    it('should delegate getEarnings to service', async () => {
      const spy = jest
        .spyOn(service, 'getEarningsOverview')
        .mockResolvedValue({ totalSubscribers: 10 } as any);
    ...
    it('should delegate submitPayoutRequest to service', async () => {
      ...
      const spy = jest
        .spyOn(service, 'submitPayoutRequest')
    ...
    it('should delegate getPayoutRequests to service', async () => {
      const spy = jest
        .spyOn(service, 'getCreatorPayoutRequests')
        .mockResolvedValue({ total: 0 } as any);
    ...
    it('should delegate getPayoutRequestById to service', async () => {
      const spy = jest
        .spyOn(service, 'getPayoutRequestById')
        .mockResolvedValue({ id: 'payout-1' } as any);
