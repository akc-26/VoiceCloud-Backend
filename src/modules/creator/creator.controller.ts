...
import { UpdateTierBenefitsDto } from './dto/update-tier-benefits.dto';
...
export class CreatorController {
  ...
  @Patch('plans/:id/benefits')
  @ApiOperation({ summary: 'Update tier benefit definitions for a creator plan' })
  @ApiParam({ name: 'id', description: 'Plan UUID' })
  @ApiResponse({ status: 200, description: 'Plan benefits updated successfully' })
  @ApiResponse({ status: 403, description: 'Unauthorized to edit this plan' })
  @ApiResponse({ status: 404, description: 'Plan not found' })
  async updatePlanBenefits(
    @CurrentUser('userId') userId: string,
    @Param('id') planId: string,
    @Body() dto: UpdateTierBenefitsDto,
  ) {
    return this.creatorService.updatePlanBenefits(userId, planId, dto.benefits);
  }
