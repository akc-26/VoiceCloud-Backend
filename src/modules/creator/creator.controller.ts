...
export class CreatorController {
  ...
  @ApiOperation({
    summary: 'Get RTMP streaming credentials and active stream key',
  })
  @ApiResponse({
    status: 200,
    description: 'Stream credentials retrieved successfully',
  })
  async getStreamCredentials(@CurrentUser('userId') userId: string) {
    return this.creatorService.getStreamCredentials(userId);
  }
  ...
  async regenerateStreamKey(@CurrentUser('userId') userId: string) {
    return this.creatorService.regenerateStreamKey(userId);
  }
}
