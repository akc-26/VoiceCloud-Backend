...
export class CreatorController {
  ...
  @ApiResponse({ status: 201, description: 'Creator plan created successfully' })
  async createPlan(
  ...
  @Get('plans')
  @ApiOperation({ summary: 'List subscription plans owned by authenticated creator' })
  @ApiResponse({ status: 200, description: 'Creator plans retrieved successfully' })
  async getMyPlans(
    @CurrentUser('userId') userId: string,
  ...
  @Public()
  @Get('plans/:creatorId')
  @ApiOperation({ summary: 'Get active public subscription plans for a creator' })
  ...
  @ApiResponse({ status: 403, description: 'Unauthorized to archive this plan' })
  @ApiResponse({ status: 404, description: 'Plan not found' })
  async deletePlan(
    @CurrentUser('userId') userId: string,
    @Param('id') planId: string,
  ...
  @ApiOperation({ summary: 'Subscribe to a creator plan (Records subscription intent)' })
  @ApiParam({ name: 'creatorId', description: 'Target creator UUID' })
  @ApiResponse({ status: 201, description: 'Subscription created successfully' })
  @ApiResponse({ status: 400, description: 'Invalid plan or self-subscription attempt' })
  @ApiResponse({ status: 409, description: 'Already actively subscribed to plan' })
  async subscribe(
    @CurrentUser('userId') userId: string,
  ...
  @ApiOperation({ summary: 'Get active subscriptions of the authenticated user' })
  ...
  @Get('subscribers')
  @ApiOperation({ summary: 'Get active subscribers for the authenticated creator' })
  @ApiResponse({ status: 200, description: 'Creator subscribers retrieved successfully' })
  ...
  // Earnings Analytics
  @Get('earnings')
  @ApiOperation({ summary: 'Get creator earnings and subscription metrics (read-only)' })
  @ApiResponse({ status: 200, description: 'Earnings metrics retrieved successfully' })
  ...
  @ApiResponse({ status: 400, description: 'Insufficient balance or below threshold' })
  @ApiResponse({ status: 409, description: 'Existing pending request in progress' })
  ...
  @ApiResponse({ status: 200, description: 'Payout request details retrieved successfully' })
  @ApiResponse({ status: 403, description: 'Unauthorized access to payout request' })
