...
import { StreamCredentialsResponseDto } from './dto/stream-credentials-response.dto';
...
export class CreatorController {
  ...
  @ApiOperation({
    summary: 'Get RTMP/WebRTC streaming credentials and active stream key',
  })
  @ApiResponse({
    status: 200,
    description: 'Stream credentials retrieved successfully',
    type: StreamCredentialsResponseDto,
  })
  @ApiResponse({
    status: 403,
    description: 'Creator account is inactive or prohibited from streaming',
  })
  async getStreamCredentials(
    @CurrentUser('userId') userId: string,
    @Ip() ipAddress: string,
  ) {
    return this.creatorService.getStreamCredentials(userId, ipAddress);
  }

  @Post('stream-credentials/regenerate')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Regenerate RTMP stream key for the authenticated creator',
  })
  @ApiResponse({
    status: 200,
  ...
  async regenerateStreamKey(
    ...
    return this.creatorService.regenerateStreamKey(userId, ipAddress);
  }
}
