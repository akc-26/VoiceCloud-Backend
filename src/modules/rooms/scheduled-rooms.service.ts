...
export class ScheduledRoomsService {
  ...
  async createScheduledRoom(
    ...
    if (isNaN(startTime.getTime()) || startTime.getTime() <= Date.now()) {
      throw new BadRequestException('Scheduled start time must be a valid future date');
    }
  ...
  async findAll(queryDto: QueryScheduledRoomDto) {
    ...
    qb.orderBy('sr.scheduledStartTime', 'ASC')
      .skip(skip)
      .take(limit);
  ...
  async updateScheduledRoom(
    ...
    if (room.hostId !== userId) {
      throw new ForbiddenException('Only the host can update this scheduled room');
    }
    ...
    if (
      ...
      throw new BadRequestException('Cannot update a cancelled or completed room');
    }
    ...
    if (updateDto.clubId) {
      ...
      if (!club) {
        throw new NotFoundException(`Club with ID "${updateDto.clubId}" not found`);
      }
    }
  ...
  async getLobbyInfo(id: string, userId?: string) {
    ...
    const countdownSeconds = Math.max(0, Math.floor((startTimeMs - Date.now()) / 1000));
  ...
  async registerReminder(
    ...
    if (
      ...
      throw new BadRequestException('Cannot set reminder for cancelled or completed room');
    }
