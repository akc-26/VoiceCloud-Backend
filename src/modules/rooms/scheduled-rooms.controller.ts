...
export class ScheduledRoomsController {
  ...
  @ApiResponse({ status: 201, description: 'Scheduled room created successfully.' })
  @ApiResponse({ status: 400, description: 'Invalid scheduled start time or parameters.' })
  async create(
  ...
  @ApiResponse({ status: 200, description: 'Scheduled rooms retrieved successfully.' })
  ...
  @Get(':id/lobby')
  @Public()
  @ApiOperation({ summary: 'Get waiting lobby information for a scheduled room' })
  ...
  @ApiResponse({ status: 201, description: 'Reminder successfully registered.' })
  @ApiResponse({ status: 400, description: 'Cannot register reminder for cancelled or completed room.' })
  ...
  @ApiResponse({ status: 200, description: 'Scheduled room details retrieved.' })
  @ApiResponse({ status: 404, description: 'Scheduled room not found.' })
  ...
  @ApiResponse({ status: 200, description: 'Scheduled room updated successfully.' })
  ...
  async update(
    ...
    return this.scheduledRoomsService.updateScheduledRoom(id, userId, updateDto);
  }

  @Delete(':id')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Cancel/delete a scheduled room' })
  @ApiResponse({ status: 200, description: 'Scheduled room cancelled successfully.' })
