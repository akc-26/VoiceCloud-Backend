...
export class SoundboardService {
  ...
  async triggerSoundEffect(
    ...
    if (room.hostId !== userId) {
      ...
      if (!isCoHost) {
        throw new ForbiddenException(
          'Only room host or co-host can trigger soundboard audio',
        );
      }
    }
    ...
    if (dto.category !== SoundEffectCategory.CUSTOM || !audioUrl) {
      const preset = this.PRESET_EFFECTS.find(
        (p) => p.category === dto.category,
      );
      if (!preset) {
        throw new BadRequestException(
          `Unknown sound effect category: ${dto.category}`,
        );
      }
    ...
    this.eventsGateway.server
      .to(`room:${roomId}`)
      .emit('room_soundboard_triggered', payload);
  ...
  async updateRoomBgm(roomId: string, userId: string, dto: UpdateRoomBgmDto) {
    ...
    if (room.hostId !== userId) {
      ...
      if (!isCoHost) {
        throw new ForbiddenException(
          'Only room host or co-host can control room background music',
        );
      }
    }
    ...
    await this.redisService.set(
      `room:${roomId}:bgm_state`,
      JSON.stringify(bgmPayload),
      86400,
    );
    ...
    this.eventsGateway.server
      .to(`room:${roomId}`)
      .emit('room_bgm_updated', bgmPayload);
