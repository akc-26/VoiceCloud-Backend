...
export class RoomTicketsService {
  ...
  async buyTicket(
    ...
    return this.dataSource.transaction(async (transactionalEntityManager) => {
      ...
      if (!room) {
        throw new NotFoundException(`Scheduled room with ID "${scheduledRoomId}" not found`);
      }
      ...
      if (
        ...
        throw new BadRequestException('Cannot purchase ticket for a cancelled or completed room');
      }

      const existingTicket = await transactionalEntityManager.findOne(RoomTicket, {
        where: {
          scheduledRoomId,
          userId,
          isValid: true,
      ...
      if (existingTicket) {
        throw new ConflictException('User already owns an active ticket for this room');
      }

      const activeTicketsCount = await transactionalEntityManager.count(RoomTicket, {
        where: {
          scheduledRoomId,
          isValid: true,
          status: TicketStatus.ACTIVE,
      ...
      if (activeTicketsCount >= room.maxParticipants) {
        throw new BadRequestException('Room capacity reached. No tickets available.');
      }
      ...
      const savedTicket = await transactionalEntityManager.save(RoomTicket, ticket);
