...
describe('Phase 2E Notifications & Remote Config Business APIs', () => {
  ...
  beforeEach(async () => {
    ...
    remoteConfigController = module.get<RemoteConfigController>(
  ...
  describe('NotificationsService - Notification Management', () => {
    ...
    it('should return notification by ID for owner', async () => {
      ...
      const res = await notificationsService.getNotificationById(
        'user-1',
        'notif-1',
      );
  ...
  describe('NotificationsController Delegation', () => {
    it('should delegate registerDevice to service', async () => {
      const spy = jest
        .spyOn(notificationsService, 'registerDevice')
        .mockResolvedValue({ id: 'dev-1' } as any);
      const dto = { deviceId: 'dev-123' };

      const res = await notificationsController.registerDevice('user-1', dto);
    ...
    it('should delegate getNotifications to service', async () => {
      ...
      const res = await notificationsController.getNotifications(
        'user-1',
        query,
      );
    ...
    it('should delegate getNotificationById to service', async () => {
      const spy = jest
        .spyOn(notificationsService, 'getNotificationById')
        .mockResolvedValue({ id: 'notif-1' } as any);

      const res = await notificationsController.getNotificationById(
        'user-1',
        'notif-1',
      );
    ...
    it('should delegate deleteNotification to service', async () => {
      ...
      const res = await notificationsController.deleteNotification(
        'user-1',
        'notif-1',
      );
  ...
  describe('RemoteConfigController Delegation', () => {
    it('should delegate getPublicRemoteConfig to service', async () => {
      const spy = jest
        .spyOn(remoteConfigService, 'getPublicRemoteConfig')
        .mockResolvedValue({ maintenanceMode: false } as any);
    ...
    it('should delegate checkVersion to service', async () => {
      const spy = jest
        .spyOn(remoteConfigService, 'checkVersion')
        .mockResolvedValue({ latestVersion: '1.2.0' } as any);
