...
describe('Notification delivery authority', () => {
  function setup() {
    ...
    const repository: any = {
      ...
      findOne: jest.fn().mockImplementation(async ({ where }) => {
        ...
        if (where.operationKey) {
          return [...rows.values()].find((row) => row.operationKey === where.operationKey) || null;
        }
        return null;
      }),
      create: jest.fn().mockImplementation((value) => ({ id: `notification-${++seq}`, ...value })),
    ...
    const service = new NotificationsService(repository, deviceRepository, gateway);
    const firebase = {
      sendSingleNotification: jest.fn().mockResolvedValue({ success: true, messageId: 'm1' }),
      sendMultiNotification: jest.fn(),
    } as any;
    const processor = new NotificationProcessor(firebase, service, deviceRepository);
  ...
  it('delivers the persisted notification and treats a repeated job as idempotent', async () => {
    ...
    expect((await service.getNotificationForDelivery(notification.id)).deliveryStatus).toBe('SENT');
  });
});
