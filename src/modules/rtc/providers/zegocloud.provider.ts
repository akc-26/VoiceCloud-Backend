...
export class ZegoCloudProvider implements IRtcProvider {
  ...
  private unavailable(operation: string): never {
    throw new ServiceUnavailableException(
      `ZEGOCLOUD ${operation} requires the official server-side ZEGOCLOUD adapter; VoiceCloud will not fabricate provider success`,
    );
  }
  ...
  verifyWebhookSignature(
    ...
    this.logger.warn(
      'ZEGOCLOUD webhook rejected because an official signature verifier is not configured',
    );
    return false;
  }
}
