...
export class AgoraProvider implements IRtcProvider {
  ...
  async refreshToken(
    ...
    this.logger.log(`[Agora] Refreshing token for user ${options.userId} in room ${options.roomId}`);
  ...
  async syncParticipantState(
    ...
    this.logger.debug(`[Agora] Syncing participant state for channel ${roomId}`);
