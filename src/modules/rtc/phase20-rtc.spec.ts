...
import { RtcSpeakerHistory, SpeakerRole } from './entities/rtc-speaker-history.entity';
import { RtcRecordingJob, RecordingJobStatus } from './entities/rtc-recording-job.entity';
...
describe('Phase 20 - Enterprise RTC Infrastructure Integration', () => {
  ...
  const mockRepository = {
    ...
    createQueryBuilder: jest.fn().mockReturnValue({
      ...
      getMany: jest.fn().mockResolvedValue([
        { rtt: 35, packetLoss: 0.5, audioLevel: 80, providerConnectionState: 'connected' },
      ]),
    }),
  };
  ...
  beforeEach(async () => {
    ...
    mockRepository.findOne.mockImplementation(({ where }) => {
      if (where?.id === 'room-101' || where?.roomId === 'room-101') return Promise.resolve(mockRoom);
      if (where?.id === 'job-505') return Promise.resolve(mockRecordingJob);
      if (where?.status === RtcSessionStatus.ACTIVE) return Promise.resolve(mockSession);
  ...
  describe('RTC Provider Abstraction & Switching', () => {
    it('should resolve registered providers (agora, livekit, zegocloud, default_mock)', () => {
      ...
      expect(factory.getProvider('zegocloud')).toBeInstanceOf(ZegoCloudProvider);
      expect(factory.getProvider('default_mock')).toBeInstanceOf(DefaultMockProvider);
    });

    it('should generate valid tokens across all providers', async () => {
      const options = { roomId: 'room-101', userId: 'user-1', role: SpeakerRole.HOST };

      const agoraToken = await factory.getProvider('agora').generateToken(mockConfig, options);
      expect(agoraToken.token).toBeDefined();
      expect(agoraToken.provider).toEqual('agora');

      const zegoToken = await factory.getProvider('zegocloud').generateToken(mockConfig, options);
      expect(zegoToken.token).toContain('ZEGO04');

      const livekitToken = await factory.getProvider('livekit').generateToken(mockConfig, options);
  ...
  describe('Room Join, Leave, Rejoin & Force Disconnect Pipeline', () => {
    it('should join room, generate token, update presence and emit participant_joined event', async () => {
      const joinDto = { roomId: 'room-101', role: SpeakerRole.SPEAKER, deviceInfo: 'ios' };
    ...
    it('should leave room, clear presence and emit participant_left event', async () => {
      ...
      expect(mockRedisClient.del).toHaveBeenCalledWith('rtc:presence:room-101:user-1');
    });
    ...
    it('should force disconnect participant by host/admin', async () => {
      ...
      expect(res.message).toContain('Participant force disconnected successfully');
      expect(mockRedisClient.del).toHaveBeenCalledWith('rtc:presence:room-101:bad-user');
    });
  });
  ...
  describe('Cloud Recording Pause & Resume Lifecycle', () => {
    it('should pause an active cloud recording job', async () => {
      const res = await service.pauseRecording('host-user-1', { jobId: 'job-505' });
    ...
    it('should resume a paused cloud recording job', async () => {
      ...
      const res = await service.resumeRecording('host-user-1', { jobId: 'job-505' });
