...
export class RtcService {
  ...
  async joinRoom(userId: string, dto: JoinRoomDto) {
    ...
    const role = dto.role || (room.hostId === userId ? SpeakerRole.HOST : SpeakerRole.LISTENER);
    ...
    if (activeSession) {
      ...
      activeSession.totalParticipants = (activeSession.totalParticipants || 1) + 1;
      await this.sessionRepository.save(activeSession);
    }
  ...
  async leaveRoom(userId: string, dto: LeaveRoomDto) {
    ...
    return { message: 'Left RTC room successfully', roomId: dto.roomId, userId };
  }
  ...
  async rejoinRoom(userId: string, dto: RejoinRoomDto) {
    ...
    return { message: 'Rejoined RTC room successfully', roomId: dto.roomId, userId };
  }

  async forceDisconnectParticipant(adminOrHostId: string, dto: ForceDisconnectDto) {
    const config = await this.getRtcConfig();
    const provider = this.providerFactory.getProvider(config.activeProvider);
    ...
    if (redis) {
      try {
        ...
        await redis.srem(`rtc:room:${dto.roomId}:participants`, dto.targetUserId);
      } catch (e) {
  ...
  async pauseRecording(userId: string, dto: PauseRecordingDto) {
    ...
    if (!job) throw new NotFoundException(`Recording job ${dto.jobId} not found`);
  ...
  async getAdminMonitoringStats() {
    ...
    const avgRtt = recentMetrics.length
      ? Math.round(recentMetrics.reduce((a, b) => a + b.rtt, 0) / recentMetrics.length)
      : 35;
    const avgPacketLoss = recentMetrics.length
      ? Number((recentMetrics.reduce((a, b) => a + b.packetLoss, 0) / recentMetrics.length).toFixed(2))
      : 0.5;

    return {
      activeRoomsCount: activeSessionsCount,
      connectedParticipantsCount: Math.max(activeSessionsCount, totalParticipants),
