  ...
  async getDashboardStats() {
    ...
    try {
      ...
      if (this.dataSource.hasMetadata('Poll')) {
        activePolls = await this.dataSource
          .getRepository('Poll')
          .count({ where: { isEnded: false } });
      }
      if (this.dataSource.hasMetadata('PollVote')) {
        totalPollVotes = await this.dataSource.getRepository('PollVote').count();
      }
    ...
    return {
      overview: {
        ...
        quizzes: { activeNow: activeQuizzes, totalParticipants: totalQuizParticipants },
