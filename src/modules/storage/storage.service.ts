...
export class StorageService {
  ...
  validateFile(file: Express.Multer.File, category: MediaCategory): void {
    ...
    // Protection against directory traversal or malicious filenames
    if (
      file.originalname &&
  ...
  async uploadFile(
    ...
    const result = await this.localStorageDriver.upload(file, dto.category);
  ...
  async replaceFile(
    ...
    await this.localStorageDriver.delete(existing.filePath);
  ...
  async deleteFile(id: string): Promise<boolean> {
    ...
    await this.localStorageDriver.delete(media.filePath);
  ...
  generatePublicUrl(filePath: string): string {
    return this.localStorageDriver.generatePublicUrl(filePath);
  }

  generateInternalUrl(filePath: string): string {
    return this.localStorageDriver.generateInternalUrl(filePath);
  }
}
