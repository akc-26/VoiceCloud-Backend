...
export class DynamicConfigService implements OnModuleInit {
  ...
  async getActiveProviderConfig(
    ...
    try {
      ...
      if (cached) {
        ...
        parsed.config = this.encryptionService.decryptConfig(parsed.config || {});
        return parsed;
      }
    } catch {
    ...
    if (provider) {
      ...
      try {
        ...
        this.logger.warn(`Failed to set Redis cache for ${cacheKey}: ${(err as Error).message}`);
      }
   ...
   */
  async getAllActiveProviders(): Promise<Record<string, ProviderConfig | null>> {
