...
export class RemoteConfigService {
  ...
  async getPublicRemoteConfig() {
    const publicSettings = (await this.settingsService.getPublicSettings()) as Record<
      string,
      any
    >;
  ...
  async checkVersion(dto: VersionCheckDto) {
    ...
    const versionRecord = await this.versionsService.findLatestByPlatform(platform);
    ...
    if (dto.currentVersion) {
      ...
      const cmpMin = this.compareVersions(dto.currentVersion, minSupportedVersion);
