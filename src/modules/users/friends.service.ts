...
export class FriendsService {
  ...
  async getPendingRequests(userId: string) {
    ...
    const allUserIds = Array.from(new Set([...allSenderIds, ...allReceiverIds]));
  ...
  async getFriendsList(userId: string, category?: string, page = 1, limit = 20) {
    ...
    const data = friends.map((f) => ({
      ...
      alias: f.alias,
      addedAt: f.createdAt,
      user: userMap.get(f.friendId) || { id: f.friendId, displayName: 'Unknown' },
    }));
  ...
  async removeFriend(userId: string, friendId: string) {
    ...
    await this.userFriendRepository.delete({ userId: friendId, friendId: userId });
    return { success: true };
  }
  ...
  async getNearbyUsers(userId: string, dto: NearbyUsersQueryDto) {
    ...
    if (dto.minLevel) {
      query.andWhere('(u.wealthLevel >= :minLevel OR u.charmLevel >= :minLevel)', {
        minLevel: dto.minLevel,
      });
    }

    query.orderBy('u.isOnline', 'DESC').addOrderBy('u.lastActiveAt', 'DESC').skip(skip).take(limit);
