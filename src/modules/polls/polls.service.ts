...
export class PollsService {
  ...
  async startPoll(userId: string, pollId: string): Promise<Poll> {
    ...
    if (poll.creatorId !== userId) {
      throw new ForbiddenException('Only the poll creator/host can start the poll');
    }
  ...
  async stopPoll(userId: string, pollId: string): Promise<Poll> {
    ...
    if (poll.creatorId !== userId) {
      throw new ForbiddenException('Only the poll creator/host can stop the poll');
    }
  ...
  async deletePoll(userId: string, pollId: string): Promise<{ success: boolean }> {
    const poll = await this.pollRepository.findOne({ where: { id: pollId } });
    if (!poll) {
      throw new NotFoundException('Poll not found');
    }
    if (poll.creatorId !== userId) {
      throw new ForbiddenException('Only the poll creator/host can delete the poll');
    }
  ...
  async vote(userId: string, pollId: string, dto: VotePollDto): Promise<Poll> {
    ...
    for (const optId of dto.optionIds) {
      if (!validOptionIds.has(optId)) {
        throw new BadRequestException(`Invalid option ID ${optId} for this poll`);
      }
    }
