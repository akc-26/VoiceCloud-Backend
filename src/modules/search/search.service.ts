...
export class SearchService {
  ...
  async globalSearch(dto: SearchQueryDto) {
    ...
    if (
      dto.type === SearchEntityType.ALL ||
      dto.type === SearchEntityType.AGENCIES
    ) {
      results.agencies = await this.searchAgencies({
        ...dto,
        q: term,
        page,
        limit,
  ...
  async searchAgencies(dto: SearchAgenciesQueryDto) {
    const page = dto.page || 1;
    const limit = dto.limit || 20;
    const skip = (page - 1) * limit;
    ...
    if (dto.q) {
      const condition = dto.prefixOnly ? `${dto.q}%` : `%${dto.q}%`;
      queryBuilder.andWhere(
        '(agency.name ILIKE :q OR agency.code ILIKE :q OR agency.description ILIKE :q)',
        { q: condition },
      );
    }

    if (dto.status) {
      queryBuilder.andWhere('agency.status = :status', { status: dto.status });
    }

    const sortOrder = dto.sortOrder || 'DESC';
    queryBuilder
      .orderBy(`agency.${dto.sortBy || 'totalRevenue'}`, sortOrder)
      .skip(skip)
      .take(limit);
