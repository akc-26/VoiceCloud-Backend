  ...
  async getListenerRetention(roomId: string) {
    ...
    return {
      ...
      retentionData: [
        ...
        { percentageTime: '25%', retentionPercentage: 88, count: Math.round(peak * 0.88) },
        { percentageTime: '50%', retentionPercentage: 76, count: Math.round(peak * 0.76) },
        { percentageTime: '75%', retentionPercentage: 68, count: Math.round(peak * 0.68) },
        { percentageTime: '100%', retentionPercentage: 60, count: Math.round(peak * 0.6) },
      ],
    };
  }
  ...
  async getGiftingHeatmap(roomId: string) {
    ...
    return {
      ...
      hourlyHeatmap: Array.from({ length: 24 }).map((_, hour) => ({
        hour,
        giftCount: hour >= 18 && hour <= 23 ? Math.round(Math.random() * 40 + 10) : Math.round(Math.random() * 10),
        coinsValue: hour >= 18 && hour <= 23 ? Math.round(Math.random() * 2000 + 500) : Math.round(Math.random() * 200),
      })),
    };
  }
