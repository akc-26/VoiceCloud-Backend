...
export class BackupScheduleService {
  ...
  async createSchedule(dto: CreateScheduleDto): Promise<BackupSchedule> {
    const schedule = this.scheduleRepository.create({
      ...
      components: dto.components || ['database', 'redis', 'storage', 'config', 'ssl'],
  ...
  async updateSchedule(id: string, dto: UpdateScheduleDto): Promise<BackupSchedule> {
    const sched = await this.findScheduleById(id);
    Object.assign(sched, dto);
    if (dto.frequency || dto.cronExpression) {
      sched.nextRunAt = this.calculateNextRun(sched.frequency, sched.cronExpression);
    }
  ...
  async processScheduledBackups() {
    this.logger.log(`[Backup Schedule Engine] Checking for due scheduled backups...`);
    const now = new Date();
    ...
    for (const sched of dueSchedules) {
      if (!sched.nextRunAt || sched.nextRunAt <= now) {
        this.logger.log(`[Backup Schedule Engine] Executing scheduled backup '${sched.name}'`);
        try {
          ...
          this.logger.error(`Failed to execute scheduled backup '${sched.name}': ${e.message}`);
        }
      }
    }
  }
