...
export class RestoreService {
  ...
  async generateRestorePreview(backupId: string) {
    ...
    if (!fs.existsSync(fullPath)) {
      throw new NotFoundException(`Backup file '${backup.filePath}' missing on server`);
    }
    ...
    if (dbEntry) {
      try {
        ...
        this.logger.warn(`Could not parse db_dump.json in backup: ${e.message}`);
      }
    }
    ...
    return {
      ...
      integrityVerified: backup.status === 'VERIFIED' || backup.status === 'COMPLETED',
    };
  }

  async restoreBackup(dto: RestoreBackupDto, operatorId = 'SYSTEM'): Promise<RestoreRecord> {
    ...
    try {
      ...
      this.logger.warn(`Failed to generate emergency safety snapshot: ${e.message}`);
    }
    ...
    try {
      ...
      const zip = new AdmZip(fullPath);
      const componentsToRestore = dto.targetComponents && dto.targetComponents.length > 0
      ...
      if (componentsToRestore.includes('database')) {
        ...
        if (dbEntry) {
          ...
          this.logger.log(`[Restore Engine] Restored database records across tables`);
        }
      }
      ...
      if (componentsToRestore.includes('storage')) {
        ...
        const storageEntries = zip.getEntries().filter(e => e.entryName.startsWith('storage/uploads/'));
      ...
      this.logger.log(`[Restore Engine] Successfully restored backup '${backup.name}' in ${restoreRecord.durationMs}ms`);
      return restoreRecord;

    } catch (err: any) {
      this.logger.error(`[Restore Engine] Restore operation failed: ${err.message}`, err.stack);

      // Rollback if requested
      if (dto.autoRollback && emergencySnapshotId) {
        this.logger.warn(`[Restore Engine] Triggering automatic rollback using emergency snapshot ${emergencySnapshotId}`);
