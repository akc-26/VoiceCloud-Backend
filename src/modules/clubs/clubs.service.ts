...
export class ClubsService {
  ...
  async createClub(userId: string, createClubDto: CreateClubDto): Promise<Club> {
    const existing = await this.clubRepository.findOne({
      where: { handle: createClubDto.handle },
    });
    if (existing) {
      throw new ConflictException(`Club handle "${createClubDto.handle}" is already taken`);
    }
  ...
  async updateClub(
    ...
    if (
      !callerMember ||
      (callerMember.role !== ClubRole.OWNER && callerMember.role !== ClubRole.ADMIN)
    ) {
      throw new ForbiddenException('Only club owner or admin can update club details');
    }
    ...
    if (updateClubDto.handle && updateClubDto.handle !== club.handle) {
      ...
      if (existing) {
        throw new ConflictException(`Club handle "${updateClubDto.handle}" is already taken`);
      }
    }
  ...
  async joinClub(
    ...
    if (club.visibility === VisibilityType.PRIVATE) {
      if (!joinDto?.inviteCode) {
        throw new ForbiddenException('Cannot join private club without a valid invitation code');
      }
    }
  ...
  async leaveClub(id: string, userId: string): Promise<{ message: string }> {
    ...
    if (member.role === ClubRole.OWNER || club.ownerId === userId) {
      throw new BadRequestException('Club owner cannot leave the club. Transfer ownership or delete the club.');
    }
  ...
  async updateMemberRole(
    ...
    if (
      ...
      throw new ForbiddenException('Only club owner or admin can update member roles');
    }
    ...
    if (targetMember.role === ClubRole.OWNER && callerUserId !== club.ownerId) {
      throw new ForbiddenException('Cannot modify owner role unless you are the owner');
    }
