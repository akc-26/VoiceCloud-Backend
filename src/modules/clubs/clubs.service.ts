...
export class ClubsService {
  ...
  async getScheduledRooms(clubId: string): Promise<ScheduledRoom[]> {
    const club = await this.clubRepository.findOne({ where: { id: clubId } });
    if (!club) {
      throw new NotFoundException(`Club with ID '${clubId}' not found`);
    }

    return this.scheduledRoomRepository.find({
      where: { clubId },
  ...
  async linkScheduledRoom(
    ...
    const scheduledRoom = await this.scheduledRoomRepository.findOne({
