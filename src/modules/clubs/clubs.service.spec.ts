...
describe('ClubsService (Phase 2A Business Layer)', () => {
  ...
  describe('createClub', () => {
    it('should create a club and assign owner as member', async () => {
      ...
      expect(clubRepo.findOne).toHaveBeenCalledWith({ where: { handle: 'tech_enthusiasts' } });
  ...
  describe('updateClub', () => {
    it('should update club if user is owner or admin', async () => {
      ...
      const result = await service.updateClub('club-123', 'user-owner', { name: 'Updated Name' });
    ...
    it('should throw ForbiddenException if user is not owner/admin', async () => {
      ...
      await expect(
        service.updateClub('club-123', 'user-regular', { name: 'Unauthorized' }),
  ...
  describe('joinClub', () => {
    ...
    it('should throw ConflictException if user is already a member', async () => {
      ...
      await expect(service.joinClub('club-123', 'user-owner')).rejects.toThrow(ConflictException);
    });
    ...
    it('should throw ForbiddenException if joining private club without invite code', async () => {
      ...
      await expect(service.joinClub('club-123', 'user-regular')).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('leaveClub', () => {
    ...
    it('should allow regular member to leave', async () => {
      const regularMember = { ...mockOwnerMember, userId: 'user-regular', role: ClubRole.MEMBER };
      clubRepo.findOne.mockResolvedValue(mockClub);
  ...
  describe('updateMemberRole', () => {
    it('should update member role when requested by owner', async () => {
      const targetMember = { ...mockOwnerMember, id: 'mem-2', userId: 'user-target', role: ClubRole.MEMBER };
      clubRepo.findOne.mockResolvedValue(mockClub);
      memberRepo.findOne
        .mockResolvedValueOnce(mockOwnerMember) // caller
        .mockResolvedValueOnce(targetMember); // target

      memberRepo.save.mockResolvedValue({ ...targetMember, role: ClubRole.ADMIN });

      const result = await service.updateMemberRole('club-123', 'user-target', 'user-owner', {
        role: ClubRole.ADMIN,
      });
