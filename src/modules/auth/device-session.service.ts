...
export class DeviceSessionService {
  ...
  async registerDevice(
    ...
    if (device) {
      device.deviceName =
        deviceInfo?.deviceName || device.deviceName || 'Mobile Device';
      device.deviceType =
        deviceInfo?.deviceType || device.deviceType || 'mobile';
  ...
  async createSession(
    ...
    if (activeSessions.length >= maxDevices) {
      ...
      for (const s of sessionsToRevoke) {
        ...
        this.logger.log(
          `Auto-revoked oldest session '${s.id}' for user '${userId}' due to max device limit (${maxDevices})`,
        );
      }
    }
    ...
    const session = this.sessionRepo.create({
      ...
      expiresAt:
        sessionDetails?.expiresAt || new Date(Date.now() + 7 * 86400 * 1000),
    });
