...
export class AuthService {
  ...
  private async checkSettingEnabled(
    key: string,
    featureName: string,
  ): Promise<void> {
    ...
    if (setting && setting.value === 'false') {
      throw new ForbiddenException(
        `${featureName} is currently disabled by system policy.`,
      );
    }
  }
  ...
  async phoneLogin(
    ...
    if (dto.otpCode) {
      ...
      throw new BadRequestException(
        'Either otpCode or firebaseIdToken is required',
      );
    }
  ...
  async googleLogin(
    ...
    const verifiedGoogle = await this.googleAuthService.verifyGoogleIdToken(
      dto.idToken,
    );
    ...
    if (!user) {
      const emailUsername = verifiedGoogle.email
        .split('@')[0]
  ...
  async upgradeGuestAccount(
    ...
    if (!user.isGuest) {
      throw new BadRequestException(
        'User account is already a registered account.',
      );
    }
    ...
    if (dto.method === 'phone' || dto.phoneNumber) {
      if (!dto.phoneNumber)
        throw new BadRequestException(
          'phoneNumber is required for phone upgrade',
        );
      if (dto.otpCode) {
        await this.otpService.verifyOtp(dto.phoneNumber, dto.otpCode);
      }
      ...
      if (!dto.googleIdToken)
        throw new BadRequestException(
          'googleIdToken is required for google upgrade',
        );
      const verified = await this.googleAuthService.verifyGoogleIdToken(
        dto.googleIdToken,
      );
  ...
  async register(
    ...
    const device = await this.deviceSessionService.registerDevice(
      savedUser.id,
      {
        ipAddress: clientIp,
      },
  ...
  async refreshToken(
    ...
    if (!user) {
      this.logger.warn(
        `[AuthDebug] Refresh token user lookup failed for identifier: ${userId} (sub=${payload.sub}, userId=${payload.userId})`,
      );
