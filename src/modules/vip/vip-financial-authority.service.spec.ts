...
describe('VipFinancialAuthorityService', () => {
  function setup() {
    ...
    const membershipRepo: any = {
      ...
      create: jest.fn().mockImplementation((value) => ({ id: 'membership-1', ...value })),
    ...
    const vipTxRepo: any = {
      findOne: jest.fn().mockImplementation(async ({ where }) => vipTransactions.get(where.operationKey) || null),
      create: jest.fn().mockImplementation((value) => ({ id: 'vip-tx-1', ...value })),
      save: jest.fn().mockImplementation(async (value) => {
    ...
    const walletTxRepo: any = {
      create: jest.fn().mockImplementation((value) => ({ id: `wallet-tx-${walletTransactions.length + 1}`, ...value })),
      save: jest.fn().mockImplementation(async (value) => {
    ...
    const claimRepo: any = {
      findOne: jest.fn().mockImplementation(async ({ where }) =>
        claims.get(`${where.userId}:${where.rewardId}:${where.periodKey}`) || null,
      ),
      create: jest.fn().mockImplementation((value) => ({ id: 'claim-1', ...value })),
      save: jest.fn().mockImplementation(async (value) => {
        claims.set(`${value.userId}:${value.rewardId}:${value.periodKey}`, value);
        return value;
      }),
    };
    ...
    const paymentFactory = { getProvider: jest.fn().mockReturnValue(provider) } as any;
    const walletMutation = {
      creditInTransaction: jest.fn().mockResolvedValue({ transaction: { id: 'vip-reward-wallet-1' } }),
    } as any;
    const redis = { del: jest.fn().mockResolvedValue(undefined) } as any;
    const notifications = { createNotification: jest.fn().mockResolvedValue({ id: 'n1' }) } as any;
    ...
    return { service, provider, walletTransactions, vipTransactions, walletMutation, membershipRepo, reward };
  }
