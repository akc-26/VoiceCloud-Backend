...
describe('Phase 2C Wallet Business APIs', () => {
  ...
  const mockPaymentProviderRepository = {
    find: jest.fn(),
    findOne: jest.fn(),
  };
  ...
  const mockCreatorSettlementRepository = {
    find: jest.fn(),
    findOne: jest.fn(),
    count: jest.fn(),
  };
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        },
        {
          ...
          useValue: mockPaymentProviderRepository,
        },
        {
        ...
        {
          provide: getRepositoryToken(CreatorSettlement),
          useValue: mockCreatorSettlementRepository,
        },
        {
    ...
    it('should return paginated transaction history', async () => {
      const mockQueryBuilder = {
        ...
        getManyAndCount: jest
          .fn()
          .mockResolvedValue([[{ id: 'tx-1', amount: 100 }], 1]),
      };
      mockWalletTransactionRepository.createQueryBuilder.mockReturnValue(
        mockQueryBuilder,
      );
  ...
  describe('WalletService - getTransactionById', () => {
    ...
    it('should throw NotFoundException if transaction is not found', async () => {
      ...
      await expect(
        service.getTransactionById('user-123', 'tx-999'),
  ...
  describe('WalletService - getCoinPackages', () => {
    it('should return active packages ordered by displayOrder', async () => {
      ...
      mockCoinPackageRepository.createQueryBuilder.mockReturnValue(
        mockQueryBuilder,
      );
  ...
  describe('WalletService - getConversionPreview', () => {
    it('should calculate conversion preview accurately without mutation', async () => {
      const preview = await service.getConversionPreview({
        diamondAmount: 150,
      });
  ...
  describe('WalletController Delegation', () => {
    it('should delegate getBalance to service', async () => {
      const spy = jest
        ...
        .mockResolvedValue({ coinBalance: 100 } as any);
    ...
    it('should delegate getSummary to service', async () => {
      const spy = jest
        .spyOn(service, 'getWalletSummary')
        .mockResolvedValue({ activePackageCount: 3 } as any);
    ...
    it('should delegate getTransactions to service', async () => {
      const spy = jest
        .spyOn(service, 'getTransactionHistory')
    ...
    it('should delegate getTransactionById to service', async () => {
      const spy = jest
        ...
        .mockResolvedValue({ id: 'tx-1' } as any);
    ...
    it('should delegate getPurchasePreview to service', async () => {
      ...
      const spy = jest
        .spyOn(service, 'getPurchasePreview')
        .mockResolvedValue({ totalCoins: 500 } as any);
    ...
    it('should delegate getConversionPreview to service', async () => {
      ...
      const spy = jest
        .spyOn(service, 'getConversionPreview')
        .mockResolvedValue({ estimatedCoins: 500 } as any);
