...
export class ReferralService {
  ...
  async getOrCreateUserReferralCode(
    ...
    if (this.redisService && userCode) {
      await this.redisService.set(cacheKey, JSON.stringify(userCode), 600);
  ...
  async getReferralSummary(userId: string) {
    ...
    const summary = {
      ...
      nextMilestone: milestones.find((m) => !m.unlocked) || null,
    };
  ...
  async applyReferralCode(referredUserId: string, dto: ApplyReferralCodeDto) {
    ...
    if (fraudStatus === FraudStatus.CLEAN) {
      await this.processRewardTriggers(savedRel.id, RewardTrigger.REGISTRATION);
    }
    ...
    // WebSocket Emission
    this.eventsGateway.broadcastToRoom(referrerId, 'referral_created', {
      relationshipId: savedRel.id,
      referredUserId,
      referralCode: cleanCode,
   ...
   */
  async processRewardTriggers(relationshipId: string, trigger: RewardTrigger) {
  ...
  async grantRewardToUser(params: {
    ...
    try {
      if (params.rewardType === RewardType.COINS && this.walletService) {
    ...
    for (const milestone of activeMilestones) {
      if (referralCount >= milestone.requiredCount) {
        ...
        if (!existingUnlock) {
          ...
          // Broadcast WS event
          this.eventsGateway.broadcastToRoom(userId, 'milestone_unlocked', {
            milestoneId: milestone.id,
  ...
  async getReferralHistory(userId: string, query: ReferralQueryDto) {
    ...
    if (query.status) {
      qb.andWhere('rel.qualificationStatus = :status', {
        status: query.status,
      });
    }
  ...
  async manualApproveReferral(relationshipId: string) {
    ...
    await this.processRewardTriggers(rel.id, RewardTrigger.REGISTRATION);
    await this.checkAndUpdateMilestones(rel.referrerId);

    this.eventsGateway.broadcastToRoom(rel.referrerId, 'referral_verified', {
      relationshipId: rel.id,
      referredUserId: rel.referredUserId,
      status: QualificationStatus.QUALIFIED,
    });
