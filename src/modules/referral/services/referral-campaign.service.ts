...
export class ReferralCampaignService {
  ...
  async updateCampaign(
    ...
    if (dto.countryRestrictions !== undefined)
      campaign.countryRestrictions = dto.countryRestrictions;
    if (dto.rewardConfiguration !== undefined)
      campaign.rewardConfiguration = dto.rewardConfiguration;
    if (dto.qualificationRules !== undefined)
      campaign.qualificationRules = dto.qualificationRules;
    if (dto.referralLimits !== undefined)
      campaign.referralLimits = dto.referralLimits;
    if (dto.dailyLimits !== undefined) campaign.dailyLimits = dto.dailyLimits;
    if (dto.globalLimits !== undefined)
      campaign.globalLimits = dto.globalLimits;
  ...
  async listCampaigns(query: ReferralQueryDto) {
    ...
    if (query.search) {
      qb.andWhere('camp.campaignName ILIKE :search', {
        search: `%${query.search}%`,
      });
    }
  ...
  async getMatchingCampaign(
    ...
    for (const campaign of activeCampaigns) {
      ...
      if (
        ...
        if (
          !country ||
          !campaign.countryRestrictions.includes(country.toUpperCase())
        ) {
          continue;
        }
      }
