...
export class ReferralFraudService {
  ...
  async analyzeReferralFraud(
    ...
    if (deviceId) {
      ...
      if (deviceCount >= 3) {
        riskScore += 50;
        reasons.push(
          `Duplicate device ID detected (${deviceCount} existing referrals)`,
        );
      }
    }
    ...
    if (ipAddress) {
      ...
      if (ipCount >= 5) {
        riskScore += 40;
        reasons.push(
          `High referral count from same IP (${ipCount} existing referrals)`,
        );
      }
    }
    ...
    if (recentReferrals >= 10) {
      riskScore += 45;
      reasons.push(
        `Excessive referral rate velocity (${recentReferrals} referrals in last hour)`,
      );
    }
  ...
  async handleFraudAction(dto: FraudActionDto) {
    if (dto.relationshipId) {
      ...
      if (rel) {
        if (
          dto.action === FraudAction.APPROVE ||
          dto.action === FraudAction.RESTORE
        ) {
          rel.fraudStatus = FraudStatus.CLEAN;
          rel.qualificationStatus = QualificationStatus.QUALIFIED;
        } else if (
          dto.action === FraudAction.REJECT ||
          dto.action === FraudAction.SUSPEND
        ) {
  ...
  async getFraudLogs(query: ReferralQueryDto) {
    ...
    if (query.search) {
      qb.andWhere(
        'log.triggerReason ILIKE :search OR log.ipAddress ILIKE :search',
        {
          search: `%${query.search}%`,
        },
      );
