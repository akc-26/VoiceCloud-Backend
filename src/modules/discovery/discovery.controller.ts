...
@ApiTags('User, Room, Host & Agency Discovery')
...
export class DiscoveryController {
  ...
  @Get('agencies/top')
  @Public()
  @ApiOperation({ summary: 'Get top agencies' })
  @ApiResponse({ status: 200, description: 'Top agencies list.' })
  async getTopAgencies(@Query() dto: DiscoveryQueryDto) {
    return this.discoveryService.getTopAgencies(dto);
  }

  @Get('agencies/fastest-growing')
  @Public()
  @ApiOperation({ summary: 'Get fastest growing agencies' })
  @ApiResponse({ status: 200, description: 'Fastest growing agencies list.' })
  async getFastestGrowingAgencies(@Query() dto: DiscoveryQueryDto) {
    return this.discoveryService.getFastestGrowingAgencies(dto);
  }

  @Get('agencies/largest')
  @Public()
  @ApiOperation({ summary: 'Get largest agencies by member count' })
  @ApiResponse({ status: 200, description: 'Largest agencies list.' })
  async getLargestAgencies(@Query() dto: DiscoveryQueryDto) {
    return this.discoveryService.getLargestAgencies(dto);
  }

  @Get('agencies/highest-earning')
  @Public()
  @ApiOperation({ summary: 'Get highest earning agencies by revenue' })
  @ApiResponse({ status: 200, description: 'Highest earning agencies list.' })
  async getHighestEarningAgencies(@Query() dto: DiscoveryQueryDto) {
    return this.discoveryService.getHighestEarningAgencies(dto);
  }
}
