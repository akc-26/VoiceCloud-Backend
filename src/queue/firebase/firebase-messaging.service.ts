...
import { App, initializeApp, cert, getApps, applicationDefault } from 'firebase-admin/app';
...
export class FirebaseMessagingService implements OnModuleInit {
  ...
  async sendSingleNotification(
    ...
    this.logger.log(`[FCM] Sending single notification to token: ${token.substring(0, 10)}...`);
    ...
    try {
      ...
      this.logger.log(`[FCM] Successfully sent single message ID: ${messageId}`);
  ...
  async sendMultiNotification(
    ...
    if (!tokens || tokens.length === 0) {
      return { success: false, error: 'NO_TOKENS_PROVIDED', successCount: 0, failureCount: 0 };
    }

    this.logger.log(`[FCM] Sending multicast notification to ${tokens.length} devices.`);
    ...
    try {
      ...
      return { success: false, error: error.message, successCount: 0, failureCount: tokens.length };
    }
  }
   ...
   */
  async sendBatchNotification(messages: FcmBatchMessage[]): Promise<FcmSendResult> {
    if (!messages || messages.length === 0) {
      return { success: false, error: 'NO_MESSAGES_PROVIDED', successCount: 0, failureCount: 0 };
    }

    this.logger.log(`[FCM] Sending batch notifications: ${messages.length} messages.`);

    let successCount = 0;
    let failureCount = 0;

    for (const msg of messages) {
      const res = await this.sendSingleNotification(msg.token, msg.notification);
      if (res.success) {
        successCount++;
      } else {
        failureCount++;
