...
import { NotificationType, Notification } from '../../modules/notifications/entities/notification.entity';
  ...
  async process(job: Job<SendPushJobData, any, string>): Promise<any> {
    ...
    try {
      ...
      if (targetTokens.length === 0) {
        this.logger.warn(`[NotificationProcessor] No valid push tokens found for job ${job.id}`);
