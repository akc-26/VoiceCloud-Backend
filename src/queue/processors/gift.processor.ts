...
export class GiftProcessor extends WorkerHost {
  ...
  private async handleAnimationDispatch(data: {
    ...
    this.logger.log(`Dispatching gift animation for queue ${data.queueId} in room ${data.roomId}`);
  ...
  private async handleSeasonalActivation(data: { giftId: string; seasonTag?: string }) {
    this.logger.log(`Activated seasonal gift ${data.giftId} (${data.seasonTag})`);
    return { status: 'activated', giftId: data.giftId };
  }

  private async handleSeasonalExpiration(data: { giftId: string; seasonTag?: string }) {
  ...
  private async handleStatisticsAggregation(data: { timeframe?: string }) {
    this.logger.log(`Aggregated gift statistics for timeframe ${data.timeframe || 'daily'}`);
