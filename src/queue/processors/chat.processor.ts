...
export class ChatProcessor extends WorkerHost {
  ...
  private async handleGenerateWaveform(data: {
    ...
    this.logger.log(`Generating waveform for voice note message ${data.messageId}`);
  ...
  private async handleTypingCleanup(data: { conversationId?: string }) {
    this.logger.log(`Cleaned up typing status for conversation ${data.conversationId}`);
