...
describe('Phase 3C Queue Processors (Workers)', () => {
  ...
  const mockUserDeviceRepo = {
    find: jest
      .fn()
      .mockResolvedValue([{ userId: 'u1', pushToken: 'token-u1' }]),
  };
  ...
  beforeEach(async () => {
    ...
    subscriptionProcessor = module.get<SubscriptionProcessor>(
      SubscriptionProcessor,
    );
  ...
  describe('RTCCleanupProcessor', () => {
    it('should clean stale Redis room state', async () => {
      ...
      expect(res.success).toBe(true);
      expect(mockRedisStateService.cleanupRoomState).toHaveBeenCalledWith(
        'room-101',
      );
    });
