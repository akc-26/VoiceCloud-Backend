...
describe('Phase 3A - Socket.IO Real-Time Extensions', () => {
  ...
  describe('1. Speaker Queue', () => {
    ...
    it('should allow listeners to join queue and receive position', async () => {
      ...
      const res = await roomGateway.handleJoinQueue(socket, { roomId, username: 'Alice' });
  ...
  describe('2. Stage Management', () => {
    ...
    it('should allow invited listener to accept invitation and join stage', async () => {
      ...
      await roomGateway.handleInviteSpeaker(hostSocket, { roomId, targetUserId: listenerId });
      const res = await roomGateway.handleAcceptInvitation(listenerSocket, { roomId });

      expect(res.success).toBe(true);
      expect(res.speaker.userId).toBe(listenerId);
      expect(mockServer.emit).toHaveBeenCalledWith('speaker_promoted', expect.anything());
    });
    ...
    it('should allow invited listener to reject invitation', async () => {
      ...
      await roomGateway.handleInviteSpeaker(hostSocket, { roomId, targetUserId: listenerId });
      const res = await roomGateway.handleRejectInvitation(listenerSocket, { roomId });

      expect(res.success).toBe(true);
      expect(mockServer.emit).toHaveBeenCalledWith('speaker_invitation_rejected', expect.anything());
    });
    ...
    it('should allow host to demote speaker to listener', async () => {
      const hostSocket = createMockSocket(hostId);
      await roomGateway.handlePromoteListener(hostSocket, { roomId, targetUserId: listenerId });
      ...
      expect(res.success).toBe(true);
      expect(mockServer.emit).toHaveBeenCalledWith('speaker_demoted', expect.anything());
    });
    ...
    it('should allow speaker to mute and unmute self', async () => {
      const hostSocket = createMockSocket(hostId);
      const muteRes = await roomGateway.handleMuteSpeaker(hostSocket, { roomId });
      expect(muteRes.success).toBe(true);
      expect(muteRes.isMuted).toBe(true);

      const unmuteRes = await roomGateway.handleUnmuteSpeaker(hostSocket, { roomId });
  ...
  describe('4. Gift Broadcast Events', () => {
    ...
    it('should broadcast gift events without altering wallet or balance', async () => {
      ...
      expect(mockServer.emit).toHaveBeenCalledWith('gift.sent', expect.anything());
      expect(mockServer.emit).toHaveBeenCalledWith('gift.received', expect.anything());
      expect(mockServer.emit).toHaveBeenCalledWith('gift.animation', expect.anything());
    });
  });
  ...
  describe('5. Room Topic Updates', () => {
    ...
    it('should allow host to update title, description, category and persist to Room entity', async () => {
      ...
      expect(res.success).toBe(true);
      expect(res.title).toBe('New AI Voice Strategy');
  ...
  describe('6. Presence Events', () => {
    ...
    it('should handle user joining presence and broadcast user_joined', async () => {
      ...
      expect(mockServer.emit).toHaveBeenCalledWith('user_joined', expect.anything());
    });
    ...
    it('should handle user leaving presence and update participant count', async () => {
      ...
      const leaveRes = await presenceGateway.handleLeavePresence(socket, { roomId });
      expect(leaveRes.success).toBe(true);
      expect(leaveRes.participantCount).toBe(0);
      expect(mockServer.emit).toHaveBeenCalledWith('user_left', expect.anything());
    });
    ...
    it('should handle user reconnection', async () => {
      ...
      expect(res.success).toBe(true);
      expect(mockServer.emit).toHaveBeenCalledWith('user_reconnected', expect.anything());
    });
    ...
    it('should return online participant count', async () => {
      ...
      const res = await presenceGateway.handleGetParticipantCount(socket, { roomId });
      expect(res.success).toBe(true);
      expect(res.participantCount).toBe(1);
    });
  });
