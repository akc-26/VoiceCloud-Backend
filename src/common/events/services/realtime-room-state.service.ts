...
import { RoomStageLayout } from '../dto/socket-payloads.dto';
import {
  RedisStateService,
  RedisQueueUser,
  RedisStageSpeaker,
...
export class RealtimeRoomStateService {
  ...
  // --- Stage 2: Stage Mute, Layout, Pin Broadcast ---

  async muteAllSpeakers(roomId: string, requesterId: string) {
    const state = await this.getOrCreateRoomState(roomId);
    if (!this.canManageStage(state, requesterId)) {
      throw {
        code: SocketErrorCode.NOT_MODERATOR,
        message: 'Only host or moderator can mute all speakers',
      };
    }

    let speakers = Array.from(state.speakers.values());
    if (speakers.length === 0 && this.redisStateService) {
      const redisSpeakers = await this.redisStateService.getSpeakers(roomId);
      speakers = redisSpeakers.map((sp) => ({
        userId: sp.userId,
        username: sp.username,
        isMuted: sp.isMuted,
        role: sp.role,
    ...
    for (const speaker of speakers) {
      if (speaker.userId !== requesterId && speaker.role !== 'host') {
        speaker.isMuted = true;
        state.speakers.set(speaker.userId, speaker);
        mutedUserIds.push(speaker.userId);
  ...
  async changeStageLayout(
    roomId: string,
    requesterId: string,
    layout: RoomStageLayout,
  ) {
    const state = await this.getOrCreateRoomState(roomId);
    if (!this.canManageStage(state, requesterId)) {
      throw {
        code: SocketErrorCode.NOT_MODERATOR,
        message: 'Only host or moderator can change layout',
      };
    }

    if (this.redisStateService) {
      await this.redisStateService.setRoomLayout(roomId, layout);
    }
  ...
  async getRoomLayout(roomId: string): Promise<string> {
    if (this.redisStateService) {
      const layout = await this.redisStateService.getRoomLayout(roomId);
      if (layout) return layout;
    }
    return RoomStageLayout.GRID;
  }
  ...
  async pinRoomMessage(
    ...
    if (!this.canManageStage(state, requesterId)) {
      throw {
        code: SocketErrorCode.NOT_MODERATOR,
        message: 'Only host or moderator can pin room messages',
      };
    }

    const pinnedData = {
      messageId: messageId || `pin_${Date.now()}`,
      message,
    ...
    if (this.redisStateService) {
      await this.redisStateService.setPinnedMessage(roomId, pinnedData);
    }
  ...
  async getPinnedMessage(roomId: string): Promise<any | null> {
    if (this.redisStateService) {
      return await this.redisStateService.getPinnedMessage(roomId);
    }
    return null;
  }
}
