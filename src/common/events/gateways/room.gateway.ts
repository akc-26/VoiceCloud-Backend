...
export class RoomGateway implements OnGatewayInit {
  ...
  @SubscribeMessage('stage:mute_all')
  async handleStageMuteAll(
    @ConnectedSocket() client: Socket,
    @MessageBody() data: MuteAllSpeakersDto,
  ) {
    const userId = this.resolveUserId(client, data);
    try {
      const res = await this.roomStateService.muteAllSpeakers(
        data.roomId,
        userId,
      );
      const payload = {
      ...
      this.server.to(data.roomId).emit('stage_all_muted', payload);
      this.server.to(data.roomId).emit('stage:all_muted', payload);
      void this.redisStateService?.publishEvent(
        'stage_all_muted',
        payload,
        data.roomId,
      ...
      return {
        success: false,
        error: err.code || 'MUTE_ALL_FAILED',
        message: err.message,
      };
    }
  }

  @SubscribeMessage('stage:change_layout')
  async handleStageChangeLayout(
    @ConnectedSocket() client: Socket,
    @MessageBody() data: ChangeStageLayoutDto,
  ) {
    const userId = this.resolveUserId(client, data);
    try {
      const res = await this.roomStateService.changeStageLayout(
        data.roomId,
        userId,
        data.layout,
      );
      this.server.to(data.roomId).emit('stage_layout_changed', res);
      this.server.to(data.roomId).emit('stage:layout_changed', res);
      void this.redisStateService?.publishEvent(
        'stage_layout_changed',
        res,
        data.roomId,
        userId,
      );
      ...
      return {
        success: false,
        error: err.code || 'CHANGE_LAYOUT_FAILED',
        message: err.message,
      };
    }
  }

  @SubscribeMessage('room:pin_message')
  async handleRoomPinMessage(
    @ConnectedSocket() client: Socket,
    @MessageBody() data: PinRoomMessageDto,
  ) {
    const userId = this.resolveUserId(client, data);
    try {
      const res = await this.roomStateService.pinRoomMessage(
        data.roomId,
        userId,
        data.message,
        data.messageId,
      );
      const payload = { roomId: data.roomId, ...res };
      this.server.to(data.roomId).emit('room_message_pinned', payload);
      this.server.to(data.roomId).emit('room:message_pinned', payload);
      void this.redisStateService?.publishEvent(
        'room_message_pinned',
        payload,
        data.roomId,
      ...
      return {
        success: false,
        error: err.code || 'PIN_MESSAGE_FAILED',
        message: err.message,
      };
    }
  }
