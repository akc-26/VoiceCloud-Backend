...
export class ReactionsGateway implements OnGatewayInit {
  ...
  private readonly EMOJI_REGEX = /^(\p{Extended_Pictographic}|\p{Emoji_Presentation})+$/u;
  ...
  handleSendEmojiReaction(
    ...
    void this.redisStateService?.publishEvent('emoji_reaction_received', payload, data.roomId, userId);
    void this.redisStateService?.publishEvent('reaction:broadcast', payload, data.roomId, userId);
  ...
  handleSendGiftEvent(
    ...
    const animPayload = {
      ...
      giftId: data.giftId,
      giftName,
      animationUrl: data.animationUrl || `https://assets.voicecloud.com/gifts/anim_${data.giftId}.json`,
      senderId,
      recipientId: data.recipientId,
      timestamp,
    };
    ...
    void this.redisStateService?.publishEvent('gift.sent', basePayload, data.roomId, senderId);

    // 2. Broadcast gift.received
    this.server.to(data.roomId).emit('gift.received', basePayload);
    void this.redisStateService?.publishEvent('gift.received', basePayload, data.roomId, senderId);

    // 3. Broadcast gift.animation
    this.server.to(data.roomId).emit('gift.animation', animPayload);
    void this.redisStateService?.publishEvent('gift.animation', animPayload, data.roomId, senderId);
