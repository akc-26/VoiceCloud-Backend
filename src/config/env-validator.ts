...
export function validateProductionEnvironment(): void {
  ...
  if (!isProduction) {
    logger.log('Development environment detected - local fallback defaults enabled.');
    return;
  }

  logger.log('Production environment detected - executing strict environment validation...');
  ...
  for (const item of requiredProductionVars) {
    ...
    if (!val || val.trim() === '' || val.includes('voicecloud_secure_jwt_secret') || val === 'localhost') {
  ...
  if (process.env.STORAGE_DRIVER === 's3') {
    if (!process.env.AWS_S3_BUCKET || !process.env.AWS_ACCESS_KEY_ID || !process.env.AWS_SECRET_ACCESS_KEY) {
      missingVars.push('AWS_S3_BUCKET / AWS_ACCESS_KEY_ID / AWS_SECRET_ACCESS_KEY (S3 Storage credentials)');
    }
  }

  if (missingVars.length > 0) {
    logger.error('CRITICAL PRODUCTION SECURITY ERROR: Missing or insecure production environment configuration!');
    missingVars.forEach((v) => logger.error(`  - Missing or default: ${v}`));
    throw new Error(`FATAL: Production startup halted due to missing environment configuration: ${missingVars.join(', ')}`);
  }
