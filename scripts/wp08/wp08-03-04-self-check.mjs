...
function expectText(path, ...needles) {
  ...
  for (const needle of needles) {
    if (!text.includes(needle))
      fail(`${path} is missing required contract text: ${needle}`);
  }
  return text;
}
function rejectText(path, ...needles) {
  ...
  for (const needle of needles) {
    if (text.includes(needle))
      fail(`${path} contains forbidden/stale contract text: ${needle}`);
  }
}
...
for (const [name, expectedHash] of acceptedMigrationHashes) {
  ...
  if (!existsSync(migrationPath))
    fail(`accepted migration is missing: ${name}`);
...
expectText(
  'creator/src/pages/WalletPage.tsx',
  'PAYOUT_USD_PER_DIAMOND = 0.005',
  'MINIMUM_PAYOUT_DIAMONDS = 100',
);
expectText(
...
const statusHeaderCount = (
  creatorWalletPage.match(/<TableCell>Status<\/TableCell>/g) || []
).length;
if (statusHeaderCount !== 1) {
  fail(
    `creator Wallet ledger must contain exactly one Status column header; found ${statusHeaderCount}`,
  );
}
expectText(
...
if (migrations.some((name) => /^1700000000014-/.test(name))) {
  fail(
    'WP08-03-04 unexpectedly introduced a schema migration; this package is UI/read integration only',
  );
}

console.log(
  'WP08-03-04 consolidated UI/real-infrastructure self-check passed.',
);
console.log(`Baseline branch: ${baselineBranch}`);
console.log(`Baseline commit: ${baselineCommit}`);
console.log(
  'Creator/Admin economy surfaces use real persisted APIs and accepted payout constants.',
);
console.log(
  'Admin gift/notification visibility is additive and read-only; no schema migration was introduced.',
);
console.log(
  'Real acceptance uses an isolated current-schema bootstrap, then replays WP08 authority migrations 0009-0013 before PostgreSQL/Redis/BullMQ/Socket.IO runtime checks.',
);
console.log(
  'All 14 accepted historical migration source files remain byte-identical.',
);
console.log(
  'package-lock.json remains byte-identical to the accepted WP08 baseline.',
);
