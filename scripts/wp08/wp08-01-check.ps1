...
Write-Host "`n[1/9] Installing locked dependencies..."
...
try {
    ...
    for ($attempt = 1; $attempt -le 90; $attempt++) {
        if ($server.HasExited) {
            throw "VoiceCloud exited before becoming ready (exit code $($server.ExitCode))."
        }
        try {
    ...
    if ($acceptance.ExitCode -ne 0) {
        throw "Real HTTP acceptance failed with exit code $($acceptance.ExitCode). Detailed acceptance and server logs are shown below and retained in the source folder."
    }
