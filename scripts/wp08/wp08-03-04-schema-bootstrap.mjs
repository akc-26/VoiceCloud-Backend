...
if (!/^voicecloud_wp08_03_04_\d{17}$/.test(databaseName)) {
  fail(`DATABASE_NAME is not a guarded WP08 temporary database: ${databaseName || '<empty>'}`);
}
if (expectedDatabaseName && expectedDatabaseName !== databaseName) {
  fail(`WP08_DATABASE_NAME (${expectedDatabaseName}) does not match DATABASE_NAME (${databaseName})`);
}
...
const historicalMigrations = [
  ...
  [1700000000007, 'Phase08HostVerificationLegacyMigrationTracking1700000000007'],
...
try {
  ...
  const [{ current_database: currentDatabase }] = await dataSource.query('SELECT current_database()');
  if (currentDatabase !== databaseName) {
    fail(`connected database ${currentDatabase} does not match guarded database ${databaseName}`);
  }

  console.log(`[BOOTSTRAP] Guarded isolated database confirmed: ${currentDatabase}`);
  console.log('[BOOTSTRAP] Creating current entity schema in the isolated database only...');
  ...
  if (Number(migrationCount) !== historicalMigrations.length) {
    fail(`expected ${historicalMigrations.length} historical migration markers, found ${migrationCount}`);
  }

  console.log(`[BOOTSTRAP] Current entity schema created (${tableCount} public tables).`);
  console.log('[BOOTSTRAP] WP08 authority delta rewound to the accepted pre-03-02 authority boundary.');
  console.log(`[BOOTSTRAP] Marked ${migrationCount} historical incremental migrations as applied.`);
  console.log('[BOOTSTRAP] Ready for migrations 1700000000009 through 1700000000013.');
} finally {
